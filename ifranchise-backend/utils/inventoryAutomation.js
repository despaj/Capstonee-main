...
async function getAllocatableStock(client, ingredientId) {
  ...
  return batches.reduce((sum, batch) => sum + Number(batch.stock || 0), 0);
}
...
function isPharmaBrand(brand) {
  return (
    brandKey(brand).includes("ipharma") || brandKey(brand).includes("pharma")
  );
}
...
function sortBatches(batches, ingredient) {
  ...
  return [...batches].sort((a, b) => {
    if (method === "FEFO") {
      const da = a.exp_date
        ? new Date(a.exp_date).getTime()
        : Number.POSITIVE_INFINITY;
      const db = b.exp_date
        ? new Date(b.exp_date).getTime()
        : Number.POSITIVE_INFINITY;
...
async function syncDirectCatalogue(client, ingredientId) {
  ...
  const extra =
    ingredient.extra_fields && typeof ingredient.extra_fields === "object"
...
async function syncIngredientFromBatches(client, ingredientId) {
  ...
  await client
    .query(`UPDATE shop_items SET stock=$1 WHERE ingredient_id=$2`, [
      totalStock,
      ingredientId,
    ])
    .catch(() => {});
...
async function allocateIngredientStock(
  client,
  ingredientId,
  requestedQuantity,
  options = {},
  ...
  for (const batch of batches) {
    ...
    const batchCost = Number(
      batch.cost_per_unit || ingredient.cost_per_unit || 0,
    );
    cogs += take * batchCost;
    allocations.push({
      batch_id: batch.id,
...
async function allocateProductStock(client, inventoryId, quantity) {
  ...
  for (const row of recipe.rows) {
    ...
    try {
      needed = convertUnit(
        needed,
        row.recipe_unit || row.ingredient_unit,
        row.ingredient_unit,
      );
    } catch {
