...
async function priceFromCost(brand, name, fallback) {
  const r = await pool.query(
    "SELECT cost_per_unit, unit, bulk_qty FROM ingredients WHERE brand=$1 AND name=$2 ORDER BY updated_at DESC LIMIT 1",
    [brand, name],
  );
  if (r.rows.length === 0) return fallback;

  const { cost_per_unit, unit, bulk_qty } = r.rows[0];

  const defaultBulkQty = ["g", "ml"].includes(unit)
    ? 1000
    : unit === "L"
      ? 200
      : unit === "kg"
        ? 50
        : ["pc", "bottle"].includes(unit)
          ? 50
          : 1;
...
router.get(
  ...
  enforceQueryScope({ branchField: "shop", brandField: "brand" }),
  async (req, res) => {
    try {
    ...
    `;
      ...
      if (brand) {
        params.push(brand);
        conditions.push(
          `LOWER(TRIM(si.brand)) = LOWER(TRIM($${params.length}))`,
        );
      }
      if (shop) {
        params.push(shop);
        conditions.push(
          `LOWER(TRIM(si.shop)) = LOWER(TRIM($${params.length}))`,
        );
      }
...
router.post(
  ...
  enforceInputScope({ branchField: "shop", brandField: "brand" }),
  async (req, res) => {
    try {
      const {
        name,
...
router.put(
  "/shop-items/:id",
  authorize("Super Admin", "Manager"),
  requireResourceScope({ table: "shop_items", branchColumn: "shop" }),
  enforceInputScope({ branchField: "shop", brandField: "brand" }),
  async (req, res) => {
    try {
      const {
      ...
      const before = await pool.query("SELECT * FROM shop_items WHERE id=$1", [
        req.params.id,
      ]);
...
router.put(
  "/shop-items/:id/toggle",
  authorize("Super Admin", "Manager"),
  requireResourceScope({ table: "shop_items", branchColumn: "shop" }),
  async (req, res) => {
    try {
      const { performed_by, latitude, longitude, performed_by_role } =
        req.body || {};

      const current = await pool.query("SELECT * FROM shop_items WHERE id=$1", [
        req.params.id,
      ]);
...
router.patch(
  "/shop-items/:id/deduct-stock",
  authorize("Super Admin", "Manager", "Staff", "Franchisee"),
  requireResourceScope({ table: "shop_items", branchColumn: "shop" }),
  async (req, res) => {
    ...
    try {
      ...
      if (item.ingredient_id) {
        ...
        if (totalAvailable < qty) {
          ...
          return res
            .status(409)
            .json({ error: "Insufficient linked ingredient stock to deduct" });
        }
      ...
      await logActivity({
        ...
        details: {
          ...
          ...(ingredientResult
            ? {
                linked_ingredient: ingredientResult.name,
                remaining_ingredient_stock: ingredientResult.stock,
              }
            : {}),
        },
        req,
...
router.delete("/shop-items/:id", authorize("Super Admin"), async (req, res) => {
  try {
    ...
    if (err.code === "23503") {
      return res.status(409).json({
        error:
          "This item can't be deleted because it's linked to past orders. Hide it instead.",
      });
    }
...
router.get(
  "/shop-item-delete-history",
  authorize("Super Admin"),
  async (req, res) => {
    try {
      const result = await pool.query(
        "SELECT * FROM shop_item_delete_history ORDER BY deleted_at DESC",
      );
      res.json(
        result.rows.map((row) => ({
          id: row.id,
      ...
      res
        .status(500)
        .json({ error: "Failed to fetch shop item delete history" });
    }
  },
);

router.delete(
  "/shop-item-delete-history/:id",
  authorize("Super Admin"),
  async (req, res) => {
    try {
      await pool.query("DELETE FROM shop_item_delete_history WHERE id=$1", [
        req.params.id,
      ]);
      res.json({ success: true });
    } catch (err) {
