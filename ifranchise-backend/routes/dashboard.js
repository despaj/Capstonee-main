  ...
  async (req, res) => {
    ...
    try {
      ...
      if (
        ...
        !transactions.length ||
        transactions.some(
          (tx) => !tx || typeof tx !== "object" || Array.isArray(tx),
        )
      ) {
        return res.status(400).json({
          success: false,
          error:
            "Select a period with valid transaction records before running AI analysis.",
        });
      }
      if (!process.env.GROQ_API_KEY?.trim()) {
        return res.status(503).json({
          success: false,
          error:
            "GROQ_API_KEY is missing from the backend environment. Set it on the backend host and restart the server.",
        });
      }
      const branchTotals = {},
      ...
      const avgBranchRev =
        sortedBranches.length > 0
          ? sortedBranches.reduce((s, [, v]) => s + v, 0) /
...
{"projectedRevenue":${weeklyRunRate},"projectedChange":5,"peakDay":"${peakDayEntry[0]}","slowestDay":"${slowestDayEntry[0]}","slowestDayDropPct":${slowestDropPct},"confidence":80,"summary":"<3 sentences>","stockAnomalies":[{"branch":"","anomalyType":"ghost_sales|low_stock_no_reorder|dead_stock","severity":"critical|warning|info","finding":"","action":""}],"recommendations":[{"branch":"","type":"success|warning|info","text":"","priority":"high|medium|low"}]}`;
                  ...
                  properties: {
                    ...
                    recommendations: {
                      ...
                      items: {
                        ...
                        required: ["branch", "type", "text", "priority"],
                      },
                    },
                  },
                  required: [
      ...
      try {
        ...
        return res.status(502).json({
          success: false,
          error: `Groq returned an unreadable response (HTTP ${response.status}). Please retry.`,
        });
      }
      if (!response.ok || data?.error) {
        const status = response.status === 429 ? 429 : 502;
        const detail = String(
          data?.error?.message || "No provider error message returned",
        )
          .split(process.env.GROQ_API_KEY)
          .join("[redacted]");
      ...
      if (choice?.finish_reason === "length") {
        return res.status(502).json({
          success: false,
          error:
            "The AI report exceeded its output limit. Try a smaller selection of branches or dates.",
        });
      }
      ...
      if (typeof raw !== "string" || !raw.trim()) {
        return res.status(502).json({
          success: false,
          error:
            "Groq returned no report content. Please retry or check the provider configuration.",
        });
      }
      let parsed;
      try {
        ...
        return res.status(502).json({
          success: false,
          error: "Groq returned invalid report JSON. Please retry.",
        });
      }
      if (
        !parsed ||
        ...
        !Array.isArray(parsed.stockAnomalies)
      ) {
        return res.status(502).json({
          success: false,
          error:
            "Groq returned an incomplete report. A summary, recommendations, and stockAnomalies are required.",
        });
      }
      ...
      console.error(
        "AI dashboard analysis failed:",
        timedOut ? "timeout" : err?.name || "Error",
      );
