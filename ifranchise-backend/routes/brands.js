...
router.get("/brand-delete-history", async (req, res) => {
  try {
    const result = await pool.query("SELECT * FROM brand_delete_history ORDER BY deleted_at DESC");
    res.json(result.rows);
  } catch (err) {
...
router.post("/brand-delete-history", async (req, res) => {
  try {
...
router.delete("/brand-delete-history/:id", async (req, res) => {
  try {
    await pool.query("DELETE FROM brand_delete_history WHERE id=$1", [req.params.id]);
    res.json({ success: true });
  } catch (err) {
