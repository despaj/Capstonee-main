/*
SETUP — run this SQL once in your database before deploying this file:

BEGIN;
ALTER TABLE announcements ADD COLUMN IF NOT EXISTS image_url TEXT;
ALTER TABLE announcements ALTER COLUMN image_url TYPE TEXT;
ALTER TABLE announcement_delete_history ADD COLUMN IF NOT EXISTS image_url TEXT;
ALTER TABLE announcement_delete_history ALTER COLUMN image_url TYPE TEXT;
COMMIT;

Replace the contents of your existing announcements.js route with this file.
The selected photo is stored in image_url as a compressed image data URL.
Existing HTTP(S) image links remain supported.

If the server returns HTTP 413, change the existing JSON parser in server.js/app.js
BEFORE your routes to: app.use(express.json({ limit: "1mb" }));
Replace the earlier parser; adding a second parser after it will not fix its limit.
The current FA photo picker compresses images to approximately 70 KB or less.
*/
...
// Logging must not turn a completed save into an apparent failure and cause retries.
...
function parsePhoto(value) {
  ...
  if (typeof value !== "string") throw new Error("Photo must be an image link or image data URL.");
  const photo = value.trim();
  if (!photo) return null;
  if (Buffer.byteLength(photo, "utf8") > 900000) throw new Error("Photo is too large. Please choose a smaller image.");
  if (/^data:image\/(jpeg|png|webp|gif);base64,[A-Za-z0-9+/]+={0,2}$/i.test(photo)) return photo;
  if (/^https?:\/\//i.test(photo)) {
    try { new URL(photo); return photo; } catch { /* rejected below */ }
  }
  if (photo.startsWith("/") && !photo.startsWith("//")) return photo;
  throw new Error("Choose a JPG, PNG, WebP, or GIF photo, or use a valid image link.");
}
...
router.post("/announcements", async (req, res) => {
  try {
    const { title, content, image_url, userId, performed_by, role, latitude, longitude, restored } = req.body || {};
    ...
    if (typeof title !== "string" || !title.trim() || typeof content !== "string" || !content.trim())
      return res.status(400).json({ error: "Please fill in the title and content fields." });

    let photo;
    try { photo = parsePhoto(image_url); }
    ...
    await logAnnouncementActivity(
      restored ? "restore" : "create", title.trim(), performed_by || "System",
    ...
    try {
      ...
      await Promise.all(allUsers.rows.map(u => pool.query(
        `INSERT INTO notifications (user_id, type, title, body, reference_id) VALUES ($1,'announcement',$2,$3,$4) ON CONFLICT DO NOTHING`,
        [u.id, title.trim(), content.length > 80 ? content.slice(0, 80) + "…" : content, announcementId]
      )));
      const tokens = await pool.query("SELECT push_token FROM users WHERE push_token IS NOT NULL");
      await Promise.all(tokens.rows.map(r => sendPushNotification(r.push_token, "New Announcement", title.trim())));
    } catch (notifErr) {
...
router.put("/announcements/:id", async (req, res) => {
  try {
    const { title, content, image_url, userId, performed_by, role, latitude, longitude } = req.body || {};
    const userResult = await pool.query("SELECT role, branch FROM users WHERE id=$1", [userId]);
    if (userResult.rows.length === 0) return res.status(404).json({ error: "User not found" });
    if (userResult.rows[0].role !== "Super Admin" && userResult.rows[0].role !== "Franchisee Operations Admin")
    ...
    const hasPhoto = Object.prototype.hasOwnProperty.call(req.body, "image_url");
    let photo = null;
    try { if (hasPhoto) photo = parsePhoto(image_url); }
    catch (err) { return res.status(400).json({ error: err.message }); }
    ...
    await logAnnouncementActivity(
      "update", title.trim(), performed_by || "System", {},
      req, userResult.rows[0].branch || null, "Announcements", latitude, longitude, role || "Unknown"
    );
...
router.delete("/announcements/:id", async (req, res) => {
  try {
    ...
    await logAnnouncementActivity(
      "delete", result.rows[0].title, performed_by || "System", {},
      req, userResult.rows[0].branch || null, "Announcements", latitude, longitude, role || "Unknown"
    );
    res.json({ success: true });
  } catch (err) {
...
router.get("/announcements-activity-log", async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT * FROM users_activity_log WHERE module = $1 ORDER BY created_at DESC", ["Announcements"]
    );
    res.json(result.rows);
  } catch (err) {
