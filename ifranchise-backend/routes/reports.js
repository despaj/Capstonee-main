...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      if (branch) {
        conditions.push(`LOWER(TRIM(branch))=LOWER(TRIM($${idx++}))`);
        params.push(branch);
      }
      if (status) {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      if (search) {
        conditions.push(
          `(r.id::text ILIKE $${idx} OR r.submitted_by ILIKE $${idx})`,
        );
        params.push(`%${search}%`);
        idx++;
      }
    ...
    try {
      ...
      if (!brand || !branch || !period || !submittedBy)
        return res.status(400).json({
          error: "brand, branch, period, and submittedBy are required",
        });
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      const result = await pool.query(
        `UPDATE reports SET status='approved', remark=NULL, updated_at=NOW() WHERE id=$1 RETURNING id, brand, branch`,
        [req.params.id],
      );
      if (result.rows.length === 0)
        return res.status(404).json({ error: "Report not found" });
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      const result = await pool.query(
        `UPDATE reports SET status='returned', remark=$1, updated_at=NOW() WHERE id=$2 RETURNING id, brand, branch`,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const check = await pool.query("SELECT id FROM reports WHERE id=$1", [
        req.params.id,
      ]);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (result.rows.length === 0)
        return res.status(404).json({ error: "Report not found" });
      await pool.query(`DELETE FROM generated_reports WHERE report_id=$1`, [
        req.params.id,
      ]);
  ...
  async (req, res) => {
    try {
      ...
      if (result.rows.length === 0)
        return res
          .status(404)
          .json({ error: "Report not found or not deleted" });
      ...
      res.json({
        success: true,
        report: await fetchReportWithComments(req.params.id),
      });
    } catch (err) {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!report) return res.status(404).json({ error: "Report not found" });
      const existing = await pool.query(
        `SELECT id FROM generated_reports WHERE report_id=$1`,
        [req.params.id],
      );
      if (existing.rows.length > 0)
      ...
      const result = await pool.query(
        ...
        [
          req.params.id,
          JSON.stringify({ ...report, content: report.content || "" }),
        ],
      );
      res.status(201).json(result.rows[0]);
    } catch (err) {
...
router.delete("/reports/:id", authorize("Super Admin"), async (req, res) => {
  try {
    const { performedBy, role, latitude, longitude } = req.body || {};
    const existing = await pool.query(
      `SELECT brand, branch FROM reports WHERE id=$1`,
      [req.params.id],
    );
    if (existing.rows.length === 0)
      return res.status(404).json({ error: "Report not found" });
    ...
    await pool.query(`DELETE FROM report_comments WHERE report_id=$1`, [
      req.params.id,
    ]);
    await pool.query(`DELETE FROM generated_reports WHERE report_id=$1`, [
      req.params.id,
