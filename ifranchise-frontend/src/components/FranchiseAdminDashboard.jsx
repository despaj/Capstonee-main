  ...
  return (
    <div className="v-kpi">
      ...
      >
        ...
        <div className={`v-kpi-icon ${color}`}>{renderedIcon}</div>
      </div>
...
export default function FranchiseeDashboard({ onLogout }) {
  ...
  useEffect(() => {
    adminModuleFetch(`${ADMIN_API_BASE}/transactions`)
      .then(async (res) => {
        if (!res.ok) {
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            ...
            {hasData ? (
              <>
                ...
                >
                  {[{ color: PAL[0], label: "Sales Revenue" }].map((l, i) => (
                    <div
                      key={i}
...
const RefreshIcon = ({ size = 13, ...props }) => (
...
function FrStockInventoryContent({ user, brands }) {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      const res = await adminModuleFetch(
        `${ADMIN_API_BASE}/ingredients?branch=${encodeURIComponent(userBranch)}`,
      );

      if (!res.ok) {
        throw new Error(`Failed to fetch ingredients: ${res.status}`);
      }
  ...
  useEffect(() => {
    ...
    adminModuleFetch(
      `${ADMIN_API_BASE}/ingredient-batches?ingredient_id=${selectedId}`,
    )
      .then((r) => r.json())
      .then((d) => {
        if (!cancelled) {
    ...
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      >
        ...
        {loading ? (
          ...
          <div
            data-fr-grid="responsive"
            className="fr-inventory-master-detail"
            style={{
              display: "grid",
...
function FrReportsContent({ user, transactions = [] }) {
    ...
    const fetchSavedReports = async () => {
      try {
        const [savedRes, liveRes] = await Promise.all([
          adminModuleFetch(
            `${ADMIN_API_BASE}/generated-reports?branch=${encodeURIComponent(branch)}`,
          ),
          adminModuleFetch(
            `${ADMIN_API_BASE}/reports?branch=${encodeURIComponent(branch)}`,
          ),
        ]);

        if (!savedRes.ok || !liveRes.ok) {
        ...
        const liveStatusMap = {};
        safeLiveData.forEach((report) => {
          liveStatusMap[report.id] = report.status;
        });

        const loaded = safeSavedData
          .map((item) => {
            let snapshot = {};
          ...
          .filter(
            (report) =>
              report.status !== "submitted" && report.status !== "deleted",
          );
  ...
  const fetchKpiStats = async (from, to) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(`Failed to fetch KPI stats: ${res.status}`);
      }
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const res = await adminModuleFetch(
          `${ADMIN_API_BASE}/reports/history?branch=${encodeURIComponent(branch)}`,
        );
  ...
  useEffect(() => {
    const fetchDeletedReports = async () => {
      ...
      try {
        const res = await adminModuleFetch(
          `${ADMIN_API_BASE}/reports/deleted?branch=${encodeURIComponent(branch)}`,
        );

        if (!res.ok) {
          throw new Error(`Failed to load deleted reports: ${res.status}`);
        }
      ...
      <div style={commStyles.header}>
        ...
        <div className="fa-header-top" style={commStyles.headerTop}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <button
              type="button"
              aria-label="Toggle announcement search"
