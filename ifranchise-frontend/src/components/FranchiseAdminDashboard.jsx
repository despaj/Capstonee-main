    ...
    <div className="fa-root">
      ...
      <aside className="fa-sidebar">
        ...
        <nav className="fa-nav">
          ...
          {!sidebarCollapsed && (
            <div className="fa-nav-section" style={{ marginTop: 8 }}>
              Account
            </div>
          )}
          {accountNav.map((item) => (
      ...
      <main className="fa-main">
        <div className="fa-topbar">
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ textAlign: "right" }}>
              ...
              <div className="fa-user-role">
                {ROLE_LABEL} - {user?.branch}
              </div>
            </div>
            <div className="fa-avatar">
              {user?.name ? user.name.trim()[0].toUpperCase() : "F"}
            </div>
          </div>
        </div>
        ...
        <div className="fa-content">
          ...
          {activeModule === "inventory" && (
            <FAMenuInventoryContent user={user} brands={brands} />
          )}
          {activeModule === "stockInventory" && (
            <StockInventoryContent user={user} brands={brands} />
          )}
          {activeModule === "mobileOrders" && (
            <FAMobileOrdersContent user={user} brands={brands} />
          )}
          {activeModule === "applications" && (
            <FAApplicationsContent
              user={user}
          ...
          {activeModule === "communication" && (
            <FACommunicationContent user={user} brands={brands} />
          )}
          {activeModule === "brandBranch" && (
            <FABrandBranchContent
              user={user}
              brands={brands}
        ...
        >
          <div
            style={{
              ...
              width: "100%",
              textAlign: "center",
              boxShadow: "0 24px 64px rgba(18,36,27,.18)",
              border: "1px solid #E1E6D8",
...
const AlertCircleIcon = ({ size = 22, color = "currentColor" }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
const InfoIcon = ({ size = 22, color = "currentColor" }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
const UploadIcon = ({ size = 28, color = "currentColor" }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
      ...
      <div ref={panelRef} style={{ padding: "18px 20px" }}>
        ...
        >
          <div style={{ display: "flex", flexDirection: "column" }}>
            <ChartLabel>
              <LineChart size={11} color="#00897b" /> Sales revenue over time ·{" "}
              {getRangeLabel()}
            </ChartLabel>
            {hasData ? (
              <>
        ...
        >
          {[
            ...
            },
            {
              ...
              value: data?.avgQty
                ? `${Number(data.avgQty).toLocaleString()} units`
                : "—",
              color: "#1e40af",
              bg: "#eff6ff",
              border: "#bfdbfe",
...
const b2bOrderItems = (o) =>
  b2bArray(o?.items || o?.order_items || o?.orderItems);
const b2bTxItems = (tx) =>
...
function B2BKpiBreakdown({
  ...
  const hqGrowth =
    previousHq > 0
      ? ((Number(overview?.hqRevenue || 0) - previousHq) / previousHq) * 100
      : null;
  const posGrowth =
    previousPos > 0
      ? ((Number(overview?.posRevenue || 0) - previousPos) / previousPos) * 100
      : null;
  const highRiskRows = branchRows.filter((r) =>
    String(r.risk || "")
      .toLowerCase()
      .includes("high"),
  );
  const watchRows = branchRows.filter((r) =>
    /watch|medium|moderate/i.test(String(r.risk || "")),
  );
  const varianceRows = skuRows.filter(
    (r) => r.stockVariance != null && Number(r.stockVariance) !== 0,
  );
  const suppliedUnits = brandRows.reduce(
    (sum, r) => sum + Number(r.suppliedQty || 0),
    0,
  );
  const soldUnits = brandRows.reduce(
    (sum, r) => sum + Number(r.soldQty || 0),
    0,
  );
  const endingUnits = brandRows.reduce(
    (sum, r) => sum + Number(r.endingStock || 0),
    0,
  );
  const revenueDifference =
    Number(overview?.posRevenue || 0) - Number(overview?.hqRevenue || 0);

  const metricMeta = {
    hqRevenue: {
      title: "HQ Supply Revenue",
      description:
        "Fulfilled and delivered Head Office supply orders for the active filters.",
      formula:
        "Sum of net amounts from fulfilled or delivered Head Office orders.",
    },
    hqPrevious: {
      title: "Previous-Month HQ Revenue",
      description:
        "The prior-month baseline used for growth and target calculations.",
      formula:
        "Sum of prior-month fulfilled or delivered Head Office supply orders.",
    },
    hqChange: {
      title: "HQ Month-on-Month Change",
      description:
        "Change in Head Office supply revenue compared with the previous month.",
      formula:
        "Current HQ revenue − previous HQ revenue; percentage uses previous HQ revenue as the base.",
    },
    posRevenue: {
      title: "Franchisee POS Revenue",
      description:
        "Paid and completed franchisee POS sales for the active filters.",
      formula:
        "Sum of net totals from non-voided, paid or completed POS transactions.",
    },
    target: {
      title: "Monthly Target",
      description:
        "The Head Office supply-revenue goal calculated from the prior month.",
      formula: `Previous HQ revenue × (1 + ${growthTargetPct}% growth target).`,
    },
    targetGap: {
      title: "Target Gap",
      description:
        "The remaining Head Office supply revenue required to reach the monthly target.",
      formula: "Maximum of zero or monthly target − current HQ supply revenue.",
    },
    coverage: {
      title: "HQ Order Coverage",
      description:
        "The portion of reported sell-through supported by authorized HQ stock flow.",
      formula:
        "Authorized HQ-supplied sellable units ÷ reported POS-sold units × 100.",
    },
    atRisk: {
      title: "At-Risk Branches",
      description:
        "Branches with high-risk or watch signals under the current filters.",
      formula:
        "Count of branches whose reconciliation rules return High Risk or Watch.",
    },
    unexplained: {
      title: "Unexplained Stock",
      description:
        "Stock variance that cannot yet be explained by authorized receipts, sales, disposal, or transfers.",
      formula:
        "Opening + HQ receipts + transfer in − POS sold − disposal − transfer out − recorded closing.",
    },
    sellThrough: {
      title: "Sell-through",
      description:
        "How much available sellable inventory was sold through POS.",
  ...
  const summariesByMetric = {
    hqRevenue: [
      ...
      [
        "Previous HQ Revenue",
        previousHq > 0 ? fmtAmt(previousHq) : "—",
        History,
        "blue",
        "Comparison baseline",
      ],
      [
        ...
        hqGrowth == null
          ? "—"
          : `${hqGrowth >= 0 ? "+" : ""}${hqGrowth.toFixed(1)}%`,
        TrendingUp,
        hqGrowth != null && hqGrowth < 0 ? "red" : "green",
        "Current vs previous",
      ],
      [
        "POS Revenue",
        fmtAmt(overview?.posRevenue),
        ShoppingCart,
        "blue",
    ...
    hqPrevious: [
      ...
      [
        "Absolute Change",
        `${Number(overview?.hqRevenue || 0) - previousHq >= 0 ? "+" : "−"}${fmtAmt(Math.abs(Number(overview?.hqRevenue || 0) - previousHq))}`,
        Activity,
        "amber",
        "Current less previous",
      ],
      [
        "MoM Growth",
        hqGrowth == null
    ...
    posRevenue: [
      ...
      [
        "Previous POS Revenue",
        previousPos > 0 ? fmtAmt(previousPos) : "—",
        History,
        "green",
        "Comparison baseline",
      ],
      [
        ...
        posGrowth == null
          ? "—"
          : `${posGrowth >= 0 ? "+" : ""}${posGrowth.toFixed(1)}%`,
        TrendingUp,
        posGrowth != null && posGrowth < 0 ? "red" : "green",
        "Current vs previous",
      ],
      [
        "POS − HQ",
        `${revenueDifference >= 0 ? "+" : "−"}${fmtAmt(Math.abs(revenueDifference))}`,
        Activity,
        "amber",
    ...
    target: [
      [
        "Monthly Target",
        fmtAmt(overview?.target),
        Target,
        "amber",
        `${growthTargetPct}% above previous HQ`,
      ],
      [
        "Current HQ Revenue",
        fmtAmt(overview?.hqRevenue),
      ...
      [
        "Target Attainment",
        overview?.targetAttainment == null
          ? "—"
          : `${Number(overview.targetAttainment).toFixed(1)}%`,
        Activity,
        "blue",
        "Current ÷ target",
      ],
      [
        "Target Gap",
        fmtAmt(overview?.targetGap),
        TrendingDown,
        Number(overview?.targetGap) > 0 ? "red" : "green",
    ...
    coverage: [
      [
        ...
        overview?.coverage == null
          ? "—"
          : `${Number(overview.coverage).toFixed(1)}%`,
        ShieldCheck,
        overview?.coverage != null && overview.coverage < 70 ? "red" : "green",
        "Authorized supply coverage",
      ],
      [
        "Branches With Coverage",
        branchRows
          .filter((r) => r.orderCoverage != null)
      ...
      [
        "Below 70%",
        branchRows
          .filter((r) => r.orderCoverage != null && r.orderCoverage < 70)
          .length.toLocaleString(),
        AlertTriangle,
        "red",
        "Needs review",
      ...
      [
        "At-Risk Branches",
        Number(overview?.atRisk || 0).toLocaleString(),
        AlertTriangle,
        "amber",
        "All active rules",
      ],
    ...
    atRisk: [
      [
        ...
        Number(overview?.atRisk || 0).toLocaleString(),
        AlertTriangle,
        Number(overview?.atRisk) > 0 ? "red" : "green",
        "High Risk + Watch",
      ],
      [
        "High Risk",
        highRiskRows.length.toLocaleString(),
        AlertTriangle,
        "red",
        "Immediate review",
      ],
      [
        "Watch",
        watchRows.length.toLocaleString(),
        Activity,
        "amber",
        "Monitor",
      ],
      [
        ...
        Math.max(
          0,
          branchRows.length - highRiskRows.length - watchRows.length,
        ).toLocaleString(),
        CheckCircle2,
        "green",
        "No current signal",
    ...
    unexplained: [
      [
        "Unexplained Units",
        overview?.unexplained == null
          ? "—"
          : Number(overview.unexplained).toLocaleString(),
        Layers,
        Number(overview?.unexplained) > 0 ? "red" : "green",
        "Absolute variance",
      ],
      [
        "Affected SKUs",
        varianceRows.length.toLocaleString(),
        Package,
        varianceRows.length ? "red" : "green",
        "Non-zero variance",
      ],
      [
        "Positive Variance",
        varianceRows
          .filter((r) => Number(r.stockVariance) > 0)
          .length.toLocaleString(),
        ArrowUp,
        "amber",
        "Recorded excess",
    ...
    sellThrough: [
      [
        "Sell-through",
        overview?.sellThrough == null
          ? "—"
          : `${Number(overview.sellThrough).toFixed(1)}%`,
        Activity,
        "green",
        "Sold ÷ available",
      ],
  ...
  let columns = [
    ...
    [
      ...
      (r) =>
        r.vsLastMonth == null
          ? "—"
          : `${r.vsLastMonth >= 0 ? "+" : ""}${r.vsLastMonth.toFixed(1)}%`,
    ],
  ...
  if (metric === "posRevenue") {
    ...
    columns = [
      ...
      [
        ...
        "right",
        (r) =>
          `${r.posRevenue - r.hqRevenue >= 0 ? "+" : "−"}${fmtAmt(Math.abs(r.posRevenue - r.hqRevenue))}`,
      ],
      ["Risk", "right", (r) => <B2BRiskBadge risk={r.risk} />],
    ];
  } else if (metric === "target" || metric === "targetGap") {
    rows = [...branchRows].sort(
      (a, b) => Number(b.targetGap || 0) - Number(a.targetGap || 0),
    );
    columns = [
      ["Branch", "left", (r) => r.branch],
      ["HQ Supply", "right", (r) => fmtAmt(r.hqRevenue)],
      [
        "Target Attainment",
        "right",
        (r) => (r.targetPct == null ? "—" : `${r.targetPct.toFixed(1)}%`),
      ],
      [
        "Target Gap",
        "right",
        (r) => (r.targetGap == null ? "—" : fmtAmt(r.targetGap)),
      ],
      ["Risk", "right", (r) => <B2BRiskBadge risk={r.risk} />],
    ];
  } else if (metric === "coverage") {
    rows = [...branchRows].sort(
      (a, b) => Number(a.orderCoverage ?? 999) - Number(b.orderCoverage ?? 999),
    );
    columns = [
      ["Branch", "left", (r) => r.branch],
      [
        ...
        "right",
        (r) =>
          r.orderCoverage == null ? "—" : `${r.orderCoverage.toFixed(1)}%`,
      ],
      ["HQ Supply", "right", (r) => fmtAmt(r.hqRevenue)],
      [
        "Stock Variance",
        ...
        (r) =>
          r.stockVariance == null
            ? "—"
            : `${r.stockVariance > 0 ? "+" : ""}${r.stockVariance.toLocaleString()}`,
      ],
      ["Risk", "right", (r) => <B2BRiskBadge risk={r.risk} />],
    ];
    ...
    columns = [
      ...
      [
        "Opening",
        "right",
        (r) => (r.openingStock == null ? "—" : r.openingStock.toLocaleString()),
      ],
      [
        "HQ Received",
        "right",
        (r) => Number(r.suppliedQty || 0).toLocaleString(),
      ],
      ["POS Sold", "right", (r) => Number(r.soldQty || 0).toLocaleString()],
      [
        "Closing",
        "right",
        (r) => (r.endingStock == null ? "—" : r.endingStock.toLocaleString()),
      ],
      [
        "Variance",
        "right",
        (r) =>
          `${r.stockVariance > 0 ? "+" : ""}${Number(r.stockVariance).toLocaleString()}`,
      ],
    ];
    emptyMessage =
      "No SKU-level variance evidence is available. Link opening stock, HQ receipts, POS deductions, disposal, transfers, and recorded closing stock.";
  } else if (metric === "sellThrough") {
    rows = [...brandRows].sort(
      (a, b) => Number(b.sellThrough || 0) - Number(a.sellThrough || 0),
    );
    entity = "brand";
    columns = [
      ...
      [
        ...
        "right",
        (r) =>
          r.endingStock == null ? "—" : Number(r.endingStock).toLocaleString(),
      ],
      [
        "Sell-through",
        "right",
        (r) =>
          r.sellThrough == null ? "—" : `${Number(r.sellThrough).toFixed(1)}%`,
      ],
    ];
    emptyMessage =
      "No brand sell-through evidence is available for the active filters.";
  }
  ...
  return (
    <div>
      ...
      >
        <div style={{ fontSize: 12, fontWeight: 850, color: "#12241B" }}>
          {meta.title}
        </div>
        <div
          style={{
        ...
        <div style={{ fontSize: 9.7, color: "#82907F", marginTop: 4 }}>
          Scope: {b2bMonthLabel(month)} · current Brand, Branch, and Risk
          filters
        </div>
      </div>
      ...
      >
        {summaries.map(([label, value, Icon, tone, note]) => (
          <B2BMetricCard
            key={label}
            label={label}
            value={value}
          ...
          <tbody>
            {rows.length ? (
              rows.slice(0, 20).map((row, index) => (
                <tr
                  key={row.id ?? `${entity}-${index}`}
                  onClick={() => openRow(row)}
                  style={{
                    cursor: "pointer",
                    background: index % 2 ? "#FBFDF9" : "#fff",
                  }}
                  onMouseEnter={(e) =>
          ...
          <div style={{ display: "grid", gap: 7 }}>
            {anomalies.slice(0, 10).map((a, i) => (
              ...
              >
                ...
                >
                  <b style={{ fontSize: 10.5, color: "#12241B" }}>
                    {a.branch || a.brand || "Unassigned"}
                  </b>
                  <B2BRiskBadge risk={a.severity} />
                </div>
                <div
                  style={{
                    fontSize: 10.2,
...
function B2BRevenueAssuranceDashboard({
    ...
    (brands || []).forEach((b) => {
      ...
      (Array.isArray(b?.branches) ? b.branches : []).forEach((br) => {
        const name =
          typeof br === "string"
            ? br
            : String(br?.name || br?.branch || br?.branch_name || "").trim();
        if (!name) return;
  ...
  const fallback = useMemo(() => {
    ...
    currentOrders.forEach((o) => {
      if (b2bBranchName(o)) allBranchNames.add(b2bBranchName(o));
    });
    currentTx.forEach((tx) => {
      if (b2bBranchName(tx)) allBranchNames.add(b2bBranchName(tx));
    });
    inventory.forEach((row) => {
      if (b2bBranchName(row)) allBranchNames.add(b2bBranchName(row));
    });
    ...
    const branchRows = Array.from(allBranchNames)
      .map((name) => {
  ...
  const branchRows = useMemo(() => {
    ...
    return branchesApi
      .map((r, i) => ({
        ...
        risk: String(
          r?.risk || r?.risk_status || r?.anomaly_status || "Normal",
        ),
  ...
  const openSku = async (row) => {
    ...
    setDrilldown({
      ...
      data: row,
      warning:
        "Full opening/receipt/POS/disposal/transfer/manual-adjustment evidence needs the B2B reconciliation endpoint and inventory movement references.",
    });
  };
  ...
  const franchisorActions = useMemo(() => {
    ...
    if (highRiskCount > 0 || Number(normalizedOverview.atRisk || 0) > 0)
      items.push({
        ...
        action:
          ...
          "Open Ghost Stock Anomalies, start with the highest-risk branch, then reconcile the affected SKU records.",
      });
    ...
    if (!items.length)
      items.push({
        ...
        evidence:
          "Current targets, risk rules, and available reconciliation evidence show no material exception.",
        action:
  ...
  return (
    <div style={{ fontFamily: FONT, marginBottom: 22 }}>
        ...
        <div style={{ ...sectionCard, marginBottom: 15, padding: "18px 19px" }}>
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 10.8, color: "#6B7A65", marginTop: 4 }}>
                ...
                <React.Fragment key={step}>
                  <span
                    style={{
                      padding: "6px 9px",
                      borderRadius: 20,
                      background: i === 0 ? "#fee2e2" : "#F6F7F1",
                      color: i === 0 ? "#991b1b" : "#526052",
                      border: `1px solid ${i === 0 ? "#fecaca" : "#DDE8DA"}`,
                      fontSize: 9.7,
                      fontWeight: 850,
                    }}
                  >
      ...
      {isOverviewView && (
        ...
        >
          ...
          <B2BMetricCard
            ...
            value={
              hasPreviousHqRevenue ? fmtAmt(normalizedOverview.target) : "—"
            }
            icon={Target}
            tone="amber"
            loading={loading}
            onClick={() => openKpi("target", "Monthly Target")}
            note={
              hasPreviousHqRevenue
                ? `${normalizedOverview.targetAttainment == null ? "—" : `${normalizedOverview.targetAttainment.toFixed(1)}%`} attained · ${growthTargetPct}% above previous HQ supply revenue`
                : "Target appears after a previous-month HQ baseline is available"
            }
          />
          <B2BMetricCard
            label="Target Gap"
            value={
              hasPreviousHqRevenue ? fmtAmt(normalizedOverview.targetGap) : "—"
            }
            icon={TrendingDown}
            tone={normalizedOverview.targetGap > 0 ? "red" : "green"}
            loading={loading}
            onClick={() => openKpi("targetGap", "Target Gap")}
            note={
              !hasPreviousHqRevenue
                ? "No previous-month baseline"
                : normalizedOverview.targetGap > 0
                  ? "HQ supply revenue still needed to hit target"
                  : "Target achieved for the selected month"
            }
          />
          <B2BMetricCard
          ...
          <B2BMetricCard
            ...
            loading={loading}
            onClick={() => openKpi("coverage", "HQ Supply Coverage")}
            note={
              normalizedOverview.coverage == null
                ? "Needs verified opening stock and inventory movements"
                : "Share of sold units supported by official stock flow"
            }
          />
          <B2BMetricCard
      ...
      {isGhostView && (
        <div className="b2b-kpi-grid" style={{ marginBottom: 15 }}>
          <B2BMetricCard
            ...
            note={
              normalizedOverview.coverage == null
                ? "Requires authorized stock-flow reconciliation"
          ...
          <B2BMetricCard
            ...
            value={
              normalizedOverview.sellThrough == null
                ? "—"
                : `${normalizedOverview.sellThrough.toFixed(1)}%`
            }
            icon={Activity}
            tone="green"
            loading={loading}
            onClick={() => openKpi("sellThrough", "Sell-through")}
            note={
              normalizedOverview.sellThrough == null
                ? "Requires sellable-unit stock flow"
                : "POS sold units ÷ sellable units available"
            }
          />
        </div>
      )}
          ...
          <div style={sectionCard}>
            ...
            >
              <div>
                ...
                <div style={{ fontSize: 10.8, color: "#6B7A65", marginTop: 3 }}>
                  Monthly supply orders versus POS revenue by store location
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                <b style={{ color: "#2c5c16" }}>
                  {monthlyLeaders.hqBranch?.branch || "—"}
                </b>
                <br />
                Highest POS:{" "}
                <b style={{ color: "#2563eb" }}>
                  {monthlyLeaders.posBranch?.branch || "—"}
                </b>
              </div>
            </div>
            <div style={tableWrap}>
            ...
            >
              ...
              >
                ...
                <b style={{ color: "#b42318" }}>
                  {monthlyLeaders.leakageBranch?.branch || "—"}
                </b>
              </div>
            </div>
            <div style={tableWrap}>
          ...
          <div style={sectionCard}>
            ...
            >
              <div>
                ...
                <div style={{ fontSize: 10.8, color: "#6B7A65", marginTop: 3 }}>
                  Highest risk first. Use coverage and stock variance to locate
                  the likely source of loss, then click a branch for evidence.
                </div>
              </div>
              <span
                ...
                <tbody>
                  {sortedBranchRows.length ? (
                      ...
                      >
                        ...
                        <td
                          style={{
                            ...
                            textAlign: "right",
                            color:
                              r.vsLastMonth != null && r.vsLastMonth < 0
                                ? "#c0392b"
                                : "#2c5c16",
                            fontWeight: 700,
                          }}
                    ...
                    <tr>
                      ...
                      >
                        No branch performance data for the selected filters.
                      </td>
                    </tr>
                  )}
                </tbody>
        ...
        <div style={{ ...sectionCard, marginBottom: 15 }}>
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 10.8, color: "#6B7A65", marginTop: 3 }}>
                HQ supply revenue, franchisee POS sell-through, quantities and
                stock variance per brand
              </div>
            </div>
            <span style={{ fontSize: 10.5, fontWeight: 800, color: "#3b791e" }}>
              ...
              <tbody>
                {brandRows.length ? (
                  brandRows.map((r, i) => (
                    ...
                    >
                      <td style={{ ...td, fontWeight: 800, color: "#12241B" }}>
                        {r.brand}
                      </td>
                      <td
                        style={{
                  ...
                  <tr>
                    ...
                    >
                      No brand performance data for the selected filters.
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      {isGhostView && (
        <div style={{ ...sectionCard, marginBottom: 15 }}>
          ...
          >
            <div>
              <div style={{ fontSize: 15, fontWeight: 800, color: "#12241B" }}>
                Sales & Stocks Reconciliation
              </div>
              <div style={{ fontSize: 10.8, color: "#6B7A65", marginTop: 3 }}>
                Quantity-level SKU comparison. Click a row for opening →
                receipts → POS → disposal → transfer → closing evidence.
              </div>
            </div>
            {!stockDataReady && (
              <span
              ...
              <thead>
                <tr>
                  {[
                    ...
                    <th
                      key={h}
                      style={{ ...th, textAlign: i < 3 ? "left" : "right" }}
                    >
                      {h}
                    </th>
                  ))}
                ...
                {skuRows.length ? (
                  skuRows.map((r, i) => (
                    ...
                    >
                      ...
                      <td style={{ ...td, fontWeight: 700 }}>{r.branch}</td>
                      <td style={{ ...td }}>{r.brand}</td>
                      <td style={{ ...td, textAlign: "right" }}>
                        {r.openingStock == null
                          ? "—"
                          : r.openingStock.toLocaleString()}
                      </td>
                      <td
                        style={{
                          ...td,
                      ...
                      <td style={{ ...td, textAlign: "right" }}>
                        {r.disposal == null && r.transfers == null
                          ? "—"
                          : b2bNum(r.disposal) + b2bNum(r.transfers)}
                      </td>
                      <td style={{ ...td, textAlign: "right" }}>
                        {r.endingStock == null
                          ? "—"
                  ...
                  <tr>
                    ...
                    >
                      No SKU order/POS activity for the selected month and
                      filters.
                    </td>
                  </tr>
                )}
      ...
      {isGhostView && (
        <div style={sectionCard}>
          ...
          >
            <div>
              <div style={{ fontSize: 15, fontWeight: 800, color: "#12241B" }}>
                Ghost Stock / Revenue Leakage
              </div>
              <div style={{ fontSize: 10.8, color: "#6B7A65", marginTop: 3 }}>
                Ranked anomalies with reason and recommended action
              </div>
            </div>
            <span
              style={{
                fontSize: 10.5,
                fontWeight: 800,
                color: anomalies.length ? "#c0392b" : "#2c5c16",
              }}
            >
          ...
          {anomalies.length ? (
            <div style={{ display: "grid", gap: 9 }}>
              {anomalies.map((a, i) => (
                <button
                  key={a.id ?? i}
                  onClick={() => {
                    const br = branchRows.find((r) => r.branch === a.branch);
                    if (br) openBranch(br);
                    else
                      setDrilldown({
                        type: "anomaly",
                  ...
                  style={{
                    display: "grid",
                    gridTemplateColumns:
                      "minmax(150px,.8fr) minmax(180px,1fr) minmax(260px,1.6fr) auto",
                    gap: 12,
                    alignItems: "center",
                    padding: "12px 13px",
                    border: "1px solid #E7EEE4",
                ...
                >
                  <div>
                    ...
                    <div
                      style={{ fontSize: 9.5, color: "#8A9687", marginTop: 2 }}
                    >
                      {[a.brand, a.sku].filter(Boolean).join(" · ") ||
                        b2bMonthLabel(month)}
                    </div>
                  </div>
                  <div>
            ...
            >
              <CheckCircle2 size={22} color="#3b791e" />
              <div
                style={{
                  fontSize: 11.5,
                  fontWeight: 800,
              ...
              >
                No anomaly detected from the data currently available.
              </div>
              <div style={{ fontSize: 10, color: "#7A887B", marginTop: 3 }}>
                {stockDataReady
                  ? "Stock reconciliation rules are included."
                  : "Stock-based anomaly rules will appear after inventory movement linkage is available."}
              </div>
            </div>
          )}
        </div>
      ...
      {drilldown && (
        <div
          onMouseDown={(e) => {
            if (e.target === e.currentTarget) setDrilldown(null);
          }}
          style={{
            position: "fixed",
            inset: 0,
        ...
        >
          ...
          >
            ...
            <div style={{ padding: 20 }}>
              {drilldown.loading ? (
                <div
                  style={{ padding: 40, textAlign: "center", color: "#6B7A65" }}
                >
                  <RefreshCw
                    size={22}
                    ...
                    <div>
                      ...
                      >
                        ...
                        <B2BMetricCard
                          ...
                          value={
                            b2bNullableNum(
                              drilldown.data?.target_attainment_pct,
                              drilldown.data?.targetPct,
                            ) == null
                              ? "—"
                              : `${b2bNullableNum(drilldown.data?.target_attainment_pct, drilldown.data?.targetPct).toFixed(1)}%`
                          }
                          icon={Target}
                          tone="amber"
                          note="Open monthly target evidence"
                          onClick={() => openKpi("target", "Target Attainment")}
                        />
                        <B2BMetricCard
                          label="Order Coverage"
                          value={
                            b2bNullableNum(
                              drilldown.data?.order_coverage,
                              drilldown.data?.orderCoverage,
                            ) == null
                              ? "—"
                              : `${b2bNullableNum(drilldown.data?.order_coverage, drilldown.data?.orderCoverage).toFixed(1)}%`
                          }
                          icon={ShieldCheck}
                          note="Open stock-flow coverage"
                          onClick={() => openKpi("coverage", "Order Coverage")}
                        />
                      </div>
                      <div
                        style={{
                    ...
                    <div>
                      ...
                      >
                        ...
                        <B2BMetricCard
                          label="Qty Supplied"
                          value={b2bNum(
                            drilldown.data?.supplied_qty,
                            drilldown.data?.suppliedQty,
                          ).toLocaleString()}
                          icon={Package}
                          note="Open HQ order coverage"
                          onClick={() =>
                            openKpi("coverage", "Quantity Supplied")
                          }
                        />
                        <B2BMetricCard
                          label="Qty Sold"
                          ...
                          icon={TrendingUp}
                          tone="blue"
                          note="Open sell-through evidence"
                          onClick={() =>
                            openKpi("sellThrough", "Quantity Sold")
                          }
                        />
                      </div>
                      <div
                      ...
                      >
                        Click an SKU in the reconciliation table for opening
                        stock, HQ receipts, POS deductions, disposal, manual
                        adjustment, closing stock and variance evidence.
                      </div>
                    </div>
                  )}
                  {drilldown.type === "sku" && (
                    ...
                    <td style={{ padding: "10px 12px" }}>
                      ...
                      >
                        ...
                        <button
                          ...
                          style={{
                            ...smallBtnSt,
                            border: isConfirm ? "none" : "1px solid #ffcdd2",
                            color: isConfirm ? C.white : "#e53935",
                            background: isConfirm ? "#e53935" : C.white,
                          }}
                        >
...
function FAMenuInventoryContent({ user, brands: propBrands = [] }) {
  ...
  const allBranches = useMemo(() => {
    ...
    brandList.forEach((b) =>
      (b.branches || []).forEach((br) => {
        ...
        if (name && !out.find((x) => x.branch === name))
  ...
  const filteredCategories = useMemo(() => {
    if (selectedBrandObj?.categories?.length)
      return selectedBrandObj.categories;
    if (filterBranch) {
      const ownerBrand = brandList.find((b) =>
        (b.branches || []).some(
          (br) => (typeof br === "string" ? br : br.name) === filterBranch,
        ),
      );
  ...
  const ViewOnlyNotice = () => (
    ...
    >
      ...
      <div style={{ fontSize: 11.5, lineHeight: 1.5 }}>
        <strong style={{ color: C.ink }}>View-only access.</strong> Franchise
        Admin can browse, search, filter, and inspect menu inventory data, but
        cannot add, edit, delete, restore, or import items.
      </div>
    </div>
  );
      ...
      <div>
        ...
        {ingredients.length === 0 ? (
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {ingredients.map((ing, idx) => (
              <span
                key={`${ing.id || ing.name || "ingredient"}-${idx}`}
                style={{
                  display: "inline-flex",
                  alignItems: "center",
                  gap: 5,
    ...
    return (
      <div style={{ fontFamily: FONT, color: C.ink }}>
        ...
        {loading && inventory.length === 0 ? (
          ...
          >
            {brandList.map((brand) => {
              const branchNames = (brand.branches || [])
                .map((br) => (typeof br === "string" ? br : br.name))
                .filter(Boolean);
              const brandItems = normalizedItems.filter((item) =>
                branchNames.includes(item.branch),
              );
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ minWidth: 0, flex: 1 }}>
                      ...
                      >
                        {branchNames.length} branch
                        {branchNames.length === 1 ? "" : "es"}
                      </div>
                    </div>
                    <div
                      style={{
  ...
  const hasActiveFilters = Boolean(
    searchQuery || filterBranch || filterCategory || filterStatus,
  );
  ...
  return (
    <div style={{ fontFamily: FONT, color: C.ink }}>
      ...
      >
        ...
        >
          <div
            style={{ position: "relative", flex: "1 1 180px", minWidth: 145 }}
          >
            <div
              style={{
                position: "absolute",
                ...
                filteredItems.map((item) => {
                  ...
                  const stockPct =
                    minStock > 0
                      ? Math.min(
                          100,
                          Math.round((currentStock / (minStock * 2)) * 100),
                        )
                      : currentStock > 0
                        ? 100
                        : 0;
      ...
      >
        ...
        {isBrand && target.branchCount > 0 && (
          ...
          >
            ...
            {target.branchCount === 1 ? "branch" : "branches"}. All branches
  ...
  return (
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <div
        style={{
          background:
            "linear-gradient(135deg,rgba(233,205,48,0.12),rgba(255,168,117,0.08))",
          border: "1.5px solid rgba(233,205,48,0.3)",
          borderRadius: 12,
          padding: "10px 16px",
          marginBottom: 18,
          display: "flex",
      ...
      >
        ...
        <span style={{ fontSize: 12, fontWeight: 600, color: "#5d4400" }}>
          View-only access — You can browse and filter stock inventory but
          cannot add, edit, import, or delete items.
        </span>
      </div>
      ...
      >
        ...
        {anyFilter && (
          ...
          >
            ...
            {brand && !branch && (
              <Chip
                label={brandList.find((b) => b.id === brand)?.name}
                color={C.greenDk}
                bg={C.greenLt}
                onRemove={() => {
                  setBrand(null);
            ...
            {unitFilter && (
              <Chip
                ...
                color="#6a1b9a"
                bg="#f3e8ff"
                onRemove={() => setUnitFilter("")}
              />
            )}
            {statusFilt && (
              <Chip
                label={statusFilt === "low" ? "Low Stock" : "In Stock"}
                color={statusFilt === "low" ? C.warn : C.ok}
                bg={statusFilt === "low" ? C.warnBg : C.okBg}
                onRemove={() => setStatusFilt("")}
              />
            )}
            <button
              onClick={clearAll}
      ...
      <div
        style={{
          ...
          borderRadius: 18,
          overflow: "hidden",
          boxShadow: "0 2px 18px rgba(0,140,60,0.07)",
        }}
      >
        <div
          style={{
        ...
        >
          ...
          <span style={{ fontSize: 12, opacity: 0.9 }}>
            {filtered.length} items · {lowCount} low stock · View-only
          </span>
        </div>
                  ...
                  {pageItems.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {"\u20B1"}
                          {Number(item.cost_per_unit || 0).toLocaleString(
                            "en-PH",
                            {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
...
const itemImage = (item) =>
  item.image || item.image_url || item.photo || item.photo_url || null;
...
function OrderStepper({ status }) {
  ...
  return (
    <div
      style={{ display: "flex", alignItems: "center", padding: "14px 4px 4px" }}
    >
      {steps.map((s, i) => {
            ...
            >
              <div
                style={{
                  ...
                  fontSize: 11,
                  fontWeight: 800,
                  background: showAsRejectedTail
                    ? "#f3f4f6"
                    : done || current
                      ? `linear-gradient(135deg,${C.teal},${C.green})`
                      : "#eef6f1",
                  color: showAsRejectedTail
                    ? "#9ca3af"
                    : done || current
                      ? "#fff"
                      : "#9db8a8",
            ...
            {!isLast && (
              <div
                style={{
                  ...
                  height: 2,
                  margin: "0 2px 18px",
                  background: !rejected && i < activeIdx ? C.green : "#e5efe8",
    ...
    >
      ...
      <select
        ...
        style={{
          ...
          height: 36,
          borderRadius: 8,
          border: `1px solid ${touched && !valid ? "#dc2626" : "#fecaca"}`,
          padding: "0 10px",
          fontSize: 12.5,
          fontFamily: "inherit",
          marginBottom: touched && !valid ? 4 : 10,
          background: "#fff",
        }}
      >
      ...
      {isAccepted && (
        <button
          ...
          style={{
            ...
            cursor: shipDisabled ? "not-allowed" : "pointer",
            display: "inline-flex",
            alignItems: "center",
            justifyContent: "center",
            gap: 6,
        ...
        >
          <thead>
            <tr style={{ borderBottom: "1px solid #000" }}>
              ...
              <th style={{ textAlign: "center", padding: "2px 0", width: 40 }}>
                Qty
              </th>
              <th style={{ textAlign: "right", padding: "2px 0", width: 70 }}>
                Amount
              </th>
            </tr>
          </thead>
...
function OrderDrawer({
  ...
  const doReject = async (reason, note) => {
    setRejecting(true);
    try {
      await onReject(order, reason, note);
      setMode(null);
    } finally {
      setRejecting(false);
    }
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {["accepted", "shipping", "received"].includes(order.status) && (
                <button
                  onClick={doPrint}
                  disabled={printing}
                  style={{ ...printBtn, opacity: printing ? 0.6 : 1 }}
                >
                  {printing ? (
                    <RefreshCw
                      size={14}
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "18px 22px" }}>
          ...
          <SectionCard title="Customer">
            {(order.brand || order.branch) && (
              ...
              >
                <Store size={14} color={C.green} /> {order.brand || "—"}
                {order.branch ? ` · ${order.branch}` : ""}
              </div>
            )}
            <div
              style={{
                fontSize: 12.5,
                color: C.muted,
                marginTop: order.brand || order.branch ? 4 : 0,
              }}
            >
              {order.customer}
            </div>
          ...
          <SectionCard title={`Items (${order.items.length})`}>
            {order.items.length === 0 ? (
              <div
                style={{ fontSize: 12, color: C.muted, fontStyle: "italic" }}
              >
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        padding: "7px 10px",
                        borderRadius: 8,
                        background: i % 2 === 0 ? "#f8fffe" : "#fff",
                        border: `1px solid ${C.greenLt}`,
                      }}
                    >
                      <div
                        style={{
          ...
          <div style={{ marginTop: 6 }}>
            ...
            {order.status === "accepted" && mode !== "reject" && (
              ...
              >
                ...
                >
                  ...
                  <span>
                    Stock was deducted on accept. Ready to send out for
                    delivery.
                  </span>
                </div>
                <div style={{ display: "flex", gap: 10 }}>
            ...
            {order.status === "shipping" && (
              ...
              >
                ...
                <span>
                  Out for delivery. Waiting for the branch to confirm receipt.
                </span>
              </div>
            )}
...
function FAMobileOrdersContent({ user, brands: propBrands = [] }) {
    ...
    async (orderList) => {
      ...
      setStockAvailability((prev) => {
        ...
        pendingOrders.forEach((o) => {
          next[o.id] = { ...(next[o.id] || {}), checking: true };
        });
        return next;
      });
      ...
      for (const order of pendingOrders) {
        ...
        setStockAvailability((prev) => ({
          ...prev,
          [order.id]: { checking: false, ok, results },
        }));
      }
    },
    [apiUrl],
  ...
  const advanceStatus = async (order, nextUiStatus, changeNote) => {
    ...
    try {
      ...
      setOrders((prev) =>
        prev.map((o) =>
          o.id === order.id ? { ...o, status: nextUiStatus } : o,
        ),
      );
      setViewOrder((v) =>
        v && v.id === order.id ? { ...v, status: nextUiStatus } : v,
      );
  ...
  const acceptOrderWithDeduction = async (order) => {
    ...
    if (!availability?.ok) {
      ...
      showToast(
        "error",
        "Not enough stock",
        list || "Insufficient stock for this order.",
      );
      return false;
    }
  ...
  const handleReject = async (order, reason, note) => {
    try {
      ...
      showToast(
        "success",
        "Order rejected",
        `#${order.id} was marked as rejected.`,
      );
    } catch {}
  };
  ...
  const handleMassAcceptAndPrint = async () => {
    const pendingOrders = orders.filter(
      (o) => o.status === "pending" && stockAvailability[o.id]?.ok,
    );
    ...
    if (pendingOrders.length === 0) {
      showToast(
        ...
        "Nothing to accept",
        skipped > 0
          ? `${skipped} order(s) skipped — insufficient stock.`
          : "No incoming orders.",
      );
      return;
    }
    ...
    showToast(
      "loading",
      "Accepting orders…",
      `Processing ${pendingOrders.length} order(s)`,
    );
    const accepted = [];
    ...
    if (accepted.length > 0) {
      showToast(
        "success",
        "Orders accepted",
        `${accepted.length} accepted${skipped ? `, ${skipped} skipped (low stock)` : ""} — sending to print.`,
      );
  ...
  const filtered = orders
    .filter((o) => {
      ...
      if (filterBranch && o.branch !== filterBranch) return false;
        ...
        filterBrand &&
        selectedBrandObj &&
        !branchOptions.includes(o.branch)
      )
        return false;
      if (search) {
        const q = search.toLowerCase();
  ...
  return (
    <div style={{ fontFamily: "'Montserrat',sans-serif" }}>
      ...
      >
        ...
        >
          <button
            onClick={handleMassAcceptAndPrint}
            disabled={massAccepting || counts.pending === 0}
            style={{
              display: "flex",
              alignItems: "center",
              gap: 6,
          ...
          >
            ...
            {massAccepting
              ? "Accepting…"
              : `Accept & Print All (${orders.filter((o) => o.status === "pending" && stockAvailability[o.id]?.ok).length})`}
          </button>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        >
          ...
          <div style={{ fontSize: 12.5, color: C.muted }}>
            {statusFilter === "all"
              ? "New orders will show up here as soon as customers place them."
      ...
      {viewOrder && (
        <OrderDrawer
          ...
          acceptDisabled={
            acceptingId === viewOrder.id ||
            (viewOrder.status === "pending" &&
              !stockAvailability[viewOrder.id]?.ok)
          }
...
function ScheduleOptionsModal({ applicant, onClose, onSend, sending }) {
  const handleSubmit = (e) => {
    ...
    const form = e.target;
    if (
      !form.optionADate.value ||
      !form.optionBDate.value ||
      !form.optionCDate.value
    )
      return;
    onSend({
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          ...
          <p style={{ fontSize: 11, color: C.muted, marginBottom: 18 }}>
            The applicant will get an email with all three times and picks
            whichever works for them — no separate confirmation step needed from
            you.
          </p>
          <div style={{ display: "flex", gap: 10 }}>
            <button
...
function ApplicationsDeleteConfirmModal({
  target,
  onConfirm,
  onClose,
  deleting = false,
...
function FAApplicationsContent({
  user,
  ...
  const handleExportCSV = () => {
    ...
    setAlertModal({
      title: `Exported ${filteredApps.length} application${filteredApps.length !== 1 ? "s" : ""}`,
      type: "success",
    });
  };
  ...
  const interviewPending = (app) => {
    ...
    const interviewEndsAt =
      new Date(app.appointmentDate).getTime() + 1 * 60 * 1000;
    return now < interviewEndsAt;
  };

  const handleSendScheduleOptions = async (
    app,
    ...
    try {
      ...
      setApplications((prev) =>
        prev.map((a) => (a.id === app.id ? { ...a, ...normalizedApp } : a)),
      );
      setViewApp((prev) =>
        prev?.id === app.id ? { ...prev, ...normalizedApp } : prev,
      );
      setMenuApp((prev) =>
        prev?.id === app.id ? { ...prev, ...normalizedApp } : prev,
      );
  ...
  const handleTestRequestReschedule = async (app) => {
    if (!app.appointmentToken) {
      setAlertModal({
        title: "No appointment token",
        message: "Schedule an interview first.",
        type: "error",
      });
      return;
    }
    setAlertModal({
      title: "Simulating client reschedule request…",
      type: "loading",
    });
    try {
      const res = await fetch(
      ...
      setViewApp((prev) =>
        prev ? { ...prev, appointmentStatus: "reschedule_requested" } : prev,
      );
      await fetchActivityLog();
                  ...
                  <>
                    {viewApp.franchise === "iPharma Mart" ? (
                      ...
                      <>
                        ...
                        <Section title="Personal Information">
                          <Field
                            label="Date of Birth"
                            value={fmtDate(viewApp.dob)}
                          />
                          <Field
                            label="Marital Status"
                            value={
                              viewApp.maritalStatus || viewApp.civil_status
                            }
                          />
                          <Field
                            label="No. of Dependents"
                            value={viewApp.dependents?.toString()}
                          />
                        ...
                        {(viewApp.spouseName || viewApp.spouseOccupation) && (
                          <Section title="Spouse Information">
                            <Field
                              label="Spouse Name"
                              value={viewApp.spouseName}
                            />
                            <Field
                              label="Spouse Occupation"
                              value={viewApp.spouseOccupation}
                            />
                            <Field
                              label="Spouse Date of Birth"
                              value={fmtDate(viewApp.spouseDob)}
                            />
                          </Section>
                        )}
                        ...
                        <Section title="Business Interest">
                          <Field
                            label="Extent of Involvement"
                            value={viewApp.involvement}
                            full
                          />
                          <Field
                            label="Equity Owned (%)"
                            value={viewApp.equity}
                          />
                          <Field
                            label="Cash Investment (₱)"
                            value={
                              viewApp.investment
                                ? `₱${Number(viewApp.investment).toLocaleString()}`
                                : null
                            }
                          />
                          <Field
                            label="Source of Funds"
                            value={viewApp.fundSource}
                          />
                          <Field
                            label="Other Businesses"
                            value={viewApp.otherBusiness}
                            full
                          />
                          <Field
                            label="Preferred Location"
                            value={viewApp.location}
                            full
                          />
                        </Section>
                        ...
                        <Section title="Declaration">
                          <Field
                            label="Family Dependence"
                            value={viewApp.familyDepend}
                            full
                          />
                          <Field
                            label="Market Area"
                            value={viewApp.marketArea}
                            full
                          />
                          <Field
                            label="Target Start Date"
                            value={fmtDate(viewApp.startDate)}
                          />
                        </Section>
                      </>
                    ) : (
                      ...
                      <>
                        <Section title="Basic Information">
                          ...
                          <Field label="Phone Number" value={viewApp.phone} />
                          <Field
                            label="Franchise Interest"
                            value={viewApp.franchise}
                          />
                          <Field
                            label="Payment Mode"
                            value={viewApp.paymentMode}
                          />
                          <Field
                            label="Date Signed"
                            value={fmtDate(viewApp.dateSigned)}
                        ...
                        <Section title="Personal Information">
                          ...
                          <Field
                            label="Civil Status"
                            value={viewApp.civilStatus}
                          />
                          <Field label="Gender" value={viewApp.gender} />
                          <Field
                            label="Nationality"
                            value={viewApp.nationality}
                          />
                          <Field
                            label="No. of Dependents"
                            value={viewApp.dependents?.toString()}
                        ...
                        <Section title="Employment Information">
                          <Field
                            ...
                            value={viewApp.employmentType}
                          />
                          <Field
                            label="Years w/ Employer"
                            value={viewApp.yearsEmployer?.toString()}
                          />
                          <Field
                            label="Monthly Income"
                            value={
                              viewApp.income
                                ? `₱${Number(viewApp.income).toLocaleString()}`
                                : null
                            }
                          />
                          ...
                          <Field
                            label="Company Name"
                            value={viewApp.employerName}
                            full
                          />
                          <Field
                            label="Business Address"
                            value={viewApp.businessAddress}
                            full
                          />
                          <Field
                            label="Nature of Business"
                            value={viewApp.businessNature}
                          />
                        </Section>
                      </>
                    )}
                    ...
                    {viewApp.appointmentDate && (
                      <Section title="Interview Appointment">
                        ...
                        <Field
                          label="Location / Mode"
                          value={viewApp.appointmentLocation}
                          full
                        />
                        <Field
                          label="Notes"
                          value={viewApp.appointmentNotes}
                          full
                        />
                      </Section>
                    )}
      ...
      {accountApp && (
        <CreateAccountModal
          ...
          onCreated={(updatedApp) => {
            ...
            setApplications((prev) =>
              prev.map((a) =>
                a.id === normalized.id ? { ...a, ...normalized } : a,
              ),
            );
            setViewApp((prev) =>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <button
                onClick={() => {
                  setViewApp(menuApp);
                  setMenuApp(null);
                  showAlert("Viewing application", null, "success");
                }}
                style={{
                  display: "flex",
                  alignItems: "center",
              ...
              <button
                onClick={() => {
                  handleApprove(menuApp.id);
                  setMenuApp(null);
                }}
                disabled={
                  menuApp.status === "approved" || processingId !== null
                }
                style={{
                  display: "flex",
                  alignItems: "center",
              ...
              <button
                onClick={() => {
                  handleReject(menuApp.id);
                  setMenuApp(null);
                }}
                disabled={
                  menuApp.status === "rejected" || processingId !== null
                }
                style={{
                  display: "flex",
                  alignItems: "center",
      ...
      <div style={{ fontFamily: "'Montserrat', sans-serif" }}>
        ...
        >
          ...
          >
            ...
            >
              {filteredApps.length} of {applications.length} application
                      ...
                      <td style={{ padding: "12px 14px" }}>
                        ...
                        {app.appointmentDate && (
                          ...
                          >
                            {app.appointmentStatus === "reschedule_requested"
                              ? "Reschedule requested"
                              : `Interview: ${new Date(app.appointmentDate).toLocaleString("en-PH", { dateStyle: "medium", timeStyle: "short", timeZone: "Asia/Manila" })}`}
                          </div>
                        )}
                      </td>
                      <td
        ...
        {scheduleApp && (
          <ScheduleOptionsModal
            ...
            onClose={() => {
              if (processingId === null) setScheduleApp(null);
            }}
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (
          ...
          onAlert(
            `An account with the email "${email}" already exists. Please use a different email or check existing accounts.`,
            "error",
          );
        } else {
    ...
    <div style={{ display: "grid", gap: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        ...
        {String(form.brand_id) ===
          brands.find((b) => b.name === "Coffee Spot")?.id?.toString() && (
          <div>
    ...
    >
      ...
      >
        ...
        {isBrand && target.branchCount > 0 && (
          ...
          >
            This will also remove {target.branchCount} associated branch
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: "#5a7a65", marginTop: 2 }}>
                    ...
                    {entry.type === "brand" && entry.data?.branches?.length > 0
                      ...
                      : ""}
                    {entry.type === "branch" && entry.brandName
                      ? ` · ${entry.brandName}`
                      : ""}
                  </div>
                </div>
...
function FABrandBranchContent({ user, brands: propBrands, onBrandsChange }) {
  ...
  const fetchActivityLog = useCallback(async () => {
    try {
      ...
      console.error("Failed to fetch brands activity log:", err);
    }
  }, []);
  ...
  const handleAddBrand = async (e) => {
    ...
    if (duplicate) {
      showError(
        "Duplicate brand",
        `A brand named "${brandForm.name}" already exists.`,
      );
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        showError("Failed to add brand", data.error || "Something went wrong.");
    } catch {
      showError(
        "Failed to add brand",
        "Something went wrong. Please try again.",
  ...
  const handleAddBranch = async (e) => {
    ...
    const parentBrand = brands.find(
      (b) => String(b.id) === String(branchForm.brand_id),
    );
    const duplicate = parentBrand?.branches?.some(
      (br) =>
        br.name.trim().toLowerCase() === branchForm.name.trim().toLowerCase(),
    );
    if (duplicate) {
      showError(
        "Duplicate branch",
        `A branch named "${branchForm.name}" already exists under this brand.`,
      );
      return;
    }
  ...
  const handleRestore = async (entry) => {
    setRestoringId(entry.id);
    showLoading(
      entry.type === "brand" ? "Restoring brand…" : "Restoring branch…",
    );
    try {
      const coords = await getBrowserLocation();
      if (entry.type === "brand") {
        ...
        showSuccess(
          "Brand restored",
          `"${brandFields.name}" has been restored.`,
        );
      } else {
        ...
        if (!parentBrand) {
          showError(
            ...
            `Parent brand "${entry.brandName || "unknown"}" was not found.`,
          );
          return;
        }
        ...
        if (data.success) {
          ...
          showSuccess(
            "Branch restored",
            `"${branchFields.name}" has been restored.`,
          );
        } else
          showError(
  ...
  const totalBranches = brands.reduce(
    (s, b) => s + (b.branches?.length || 0),
    0,
  );
  const allRegions = [
    ...new Set(
      brands
        .flatMap((b) => b.branches?.map((br) => br.region) || [])
        .filter(Boolean),
    ),
  ];
  const filteredBrands = brands
    ...
    .filter((brand) => {
      if (filterBrand !== "all" && String(brand.id) !== String(filterBrand))
        return false;
      if (
        filterRegion === "all" &&
        searchQuery &&
        ...
        >
          {[
            ...
            },
            {
              ...
              value: totalBranches,
              icon: <Store size={20} color="#065f46" />,
              bg: "linear-gradient(135deg,#d1fae5,#a7f3d0)",
            ...
            <div key={i} className="bm-stat">
              ...
              >
                <div>
                  ...
                  <div
                    style={{ fontSize: 24, fontWeight: 800, color: "#0d2b1e" }}
                  >
                    {s.value}
                  </div>
                </div>
        ...
        >
          ...
          >
            ...
            <div style={{ marginLeft: "auto", display: "flex", gap: 10 }}>
              ...
              >
                <History size={14} /> Delete History
                {deletedHistory.length > 0 ? ` (${deletedHistory.length})` : ""}
              </button>
              <button
                onClick={() => {
                  setBranchForm(emptyBranch);
                  ...
                  <div>
                    ...
                    >
                      {brand.contact_email && (
                        ...
                        >
                          <Mail size={11} /> {brand.contact_email}
                        </span>
                      )}
                      {brand.contact_phone && (
                        <span
                        ...
                        >
                          <Phone size={11} /> {brand.contact_phone}
                        </span>
                      )}
                    </div>
                  </div>
