...
@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap');
    ...
    .fr-detail-dialog { animation:adDetailEnter .18s ease-out; }
    .fr-detail-dialog button, .fr-detail-dialog summary {
      transition:transform .14s ease, background-color .14s ease, box-shadow .14s ease;
    }
    .fr-detail-dialog button:not(:disabled):hover { box-shadow:0 2px 7px rgba(44,92,22,.12); }
    .fr-detail-dialog button:not(:disabled):active, .fr-detail-dialog summary:active { transform:scale(.97); }
    .fr-detail-dialog button:focus-visible, .fr-detail-dialog summary:focus-visible {
      outline:2px solid #3b791e; outline-offset:3px;
    }
    .fr-detail-dialog button:disabled { opacity:.45; cursor:not-allowed; }
    .fr-evidence-view, .fr-detail-dialog details[open] .fr-order-lines { animation:adEvidenceEnter .16s ease-out; }
    ...
    @media (prefers-reduced-motion:reduce) {
      .fr-detail-dialog, .fr-detail-dialog *, .fr-evidence-view { animation:none!important;transition:none!important;transform:none!important; }
    }
    * { margin:0; padding:0; box-sizing:border-box; }
    :root {
      --g1:#b3a941; --g2:#3b791e; --g3:#2c5c16; --g4:#12241B;
    ...
    .fr-topbar-title { font-family:'Plus Jakarta Sans',sans-serif; color:#12241B; font-size: 22px; font-weight: 800; }
    .fr-avatar {
    ...
    .fr-user-name { font-weight: 700; font-size: 13px; color: #12241B; text-align: right; }
    .fr-user-role { font-size: 11.5px; color: #5C6B60; text-align: right; }

    .fr-branch-change-row { transition:background-color .15s ease; }
    .fr-branch-change-row:hover, .fr-branch-change-row:focus-visible { background:#f0f5e8 !important; }
    .fr-branch-change-row:active { background:#e3edd6; }
    ...
    body.fr-admin-ui .franchisee-root { background:#F6F7F1 !important; background-image:none !important; }
  ...
  .franchisee-root { display:flex; min-height:100vh; color:#12241B; background:#F6F7F1; }
  .fr-sidebar { width:${sidebarCollapsed ? "76px" : "272px"}; height:100dvh; overflow-x:hidden; z-index:1000; transition:width .3s ease,transform .3s ease; }
  .fr-main { flex:1; min-width:0; margin-left:${sidebarCollapsed ? "76px" : "272px"}; transition:margin-left .3s ease; }
  .fr-content { width:100%; max-width:1400px; margin:0 auto; padding:20px 30px 40px; min-width:0; }
  .fr-sidebar-header img { max-width:100%; }
  .fr-logo-mark { background:transparent; margin:auto; }
  .fr-logo-mark img { width:35px; height:35px; object-fit:contain; }
  .fr-nav-label { display:${sidebarCollapsed ? "none" : "block"}; text-align:left; }
  .fr-topbar { min-height:72px; z-index:100; gap:12px; }
  .fr-topbar-heading { display:flex; align-items:center; gap:10px; min-width:0; }
  .fr-topbar-title { overflow-wrap:anywhere; }
  body.fr-admin-ui button.fr-nav-item { width:100%; justify-content:${sidebarCollapsed ? "center" : "flex-start"}; text-align:left; border:0; background:transparent; color:#5C6B60; padding:10px 12px; gap:12px; min-height:40px; }
  body.fr-admin-ui button.fr-nav-item.active { background:#F6F7F1; color:#2c5c16; font-weight:700!important; }
  body.fr-admin-ui button.fr-nav-item:hover { background:#F6F7F1; color:#12241B; }
  body.fr-admin-ui .fr-nav-item.logout { color:#c0392b; }
  body.fr-admin-ui .fr-nav-item.logout:hover { background:#fdf1f0; }
  body.fr-admin-ui .fr-nav-icon svg { width:20px; height:20px; }
  body.fr-admin-ui .fr-avatar { background:#12241B; color:#b3a941; border:0; border-radius:12px!important; flex-shrink:0; }
  body.fr-admin-ui .fr-mobile-menu { display:none; }
  .fr-sidebar-backdrop { display:none; }
  @media(max-width:900px) {
    .fr-sidebar { width:272px; max-width:calc(100vw - 48px); transform:translateX(${sidebarCollapsed ? "-100%" : "0"}); visibility:${sidebarCollapsed ? "hidden" : "visible"}; box-shadow:8px 0 30px rgba(14,59,34,.12); }
    .fr-main { margin-left:0; width:100%; }
    .fr-nav-label { display:block; }
    body.fr-admin-ui button.fr-nav-item { justify-content:flex-start; }
    .fr-sidebar-backdrop { display:block; position:fixed; inset:0; background:rgba(18,36,27,.42); z-index:999; }
    body.fr-admin-ui .fr-mobile-menu { display:inline-flex; width:38px; height:38px; padding:0; flex-shrink:0; }
    .fr-topbar { padding:12px 16px; min-height:64px; }
    .fr-content { padding:16px; }
    .fr-topbar-title { font-size:18px; }
    .fr-user-name,.fr-user-role { display:none; }
  }
  @media(max-width:560px) {
    .fr-content { padding:12px; }
    .fr-topbar { padding:10px 12px; }
    .fr-topbar-title { font-size:17px; }
  }
`;

const FR_MODULE_PARITY_CSS = `  /* Admin surface, controls and typography, shared by every franchisee module. */
  body.fr-admin-ui .fr-content { --fr-card-shadow:0 2px 12px rgba(50,109,32,.06); }
  body.fr-admin-ui .v-card, body.fr-admin-ui .v-kpi { border-color:#E1E6D8!important; background:#fff!important; box-shadow:var(--fr-card-shadow)!important; }
  body.fr-admin-ui .v-card { border-radius:18px!important; }
  body.fr-admin-ui .v-kpi { border-radius:16px!important; padding:16px 17px; min-width:0; }
  body.fr-admin-ui .v-kpi::before { display:none; }
  body.fr-admin-ui .v-kpi-label { color:#5C6B60; font-size:10px; letter-spacing:.07em; }
  body.fr-admin-ui .v-kpi-value { font-size:26px; color:#12241B; overflow-wrap:anywhere; }
  body.fr-admin-ui .v-kpi-icon { width:36px; height:36px; border-radius:11px; }
  body.fr-admin-ui .v-kpi-icon.green { background:#eef7e9; color:#3b791e; }
  body.fr-admin-ui .v-kpi-icon.blue { background:#eff6ff; color:#2563eb; }
  body.fr-admin-ui .v-kpi-icon.orange { background:#fff7ed; color:#b45309; }
  body.fr-admin-ui .v-kpi-icon.red { background:#fdf1f0; color:#c0392b; }
  body.fr-admin-ui .v-section-head { gap:14px; flex-wrap:wrap; border-bottom:1px solid #E1E6D8; }
  body.fr-admin-ui .v-section-title { font-size:14px; color:#347022; }
  body.fr-admin-ui .v-section-title-accent { display:none; }
  body.fr-admin-ui .v-btn-primary { background:#3b791e!important; color:#fff!important; border-color:#3b791e!important; }
  body.fr-admin-ui .v-btn-primary:not(:disabled):hover { background:#509820!important; }
  body.fr-admin-ui .v-btn-secondary, body.fr-admin-ui .v-btn-ghost { background:#fff; color:#2c5c16; border-color:#E1E6D8; }
  body.fr-admin-ui .v-btn-danger { background:#c0392b; color:#fff; border-color:#c0392b; }
  body.fr-admin-ui .v-btn-blue { background:#f0f5e8; color:#2c5c16; border-color:#c9dba0; }
  body.fr-admin-ui .v-form-input,body.fr-admin-ui .v-form-select,body.fr-admin-ui .v-search { min-height:38px; border-radius:11px!important; border:1.5px solid #E1E6D8!important; color:#347022!important; background:#fff!important; font-size:13px!important; }
  body.fr-admin-ui .v-form-input:focus,body.fr-admin-ui .v-form-select:focus,body.fr-admin-ui .v-search:focus { border-color:#3b791e!important; box-shadow:0 0 0 3px rgba(59,121,30,.1)!important; }
  body.fr-admin-ui .v-form-input:disabled { background:#F3F4F1!important; color:#5C6B60!important; }
  body.fr-admin-ui .v-table th { background:#f0f5e8!important; color:#5C6B60!important; border-bottom:1px solid #E1E6D8!important; padding:12px 14px!important; font-size:10px!important; text-align:left!important; }
  body.fr-admin-ui .v-table td { padding:12px 14px!important; font-size:12px!important; border-color:#E1E6D8; }
  body.fr-admin-ui .v-table tbody tr:hover td { background:#F6F7F1; }
  body.fr-admin-ui .v-table { min-width:600px; }
  body.fr-admin-ui .v-tabs { max-width:100%; flex-wrap:wrap; }
  body.fr-admin-ui .v-tab { background:transparent; border:0; color:#5C6B60; }
  body.fr-admin-ui .v-tab.active { background:#3b791e; color:#fff; }
  body.fr-admin-ui .v-modal-overlay { padding:16px; background:rgba(0,0,0,.55); }
  body.fr-admin-ui .v-modal { border-radius:22px; max-width:calc(100vw - 32px); max-height:calc(100dvh - 32px); overscroll-behavior:contain; }
  body.fr-admin-ui .fr-db-kpi { border:1px solid #E1E6D8; border-radius:16px; padding:16px 17px; box-shadow:var(--fr-card-shadow); min-height:126px; min-width:0; }
  body.fr-admin-ui .fr-db-kpi-grid { grid-template-columns:repeat(4,minmax(0,1fr)); gap:12px; }
  body.fr-admin-ui .fr-db-chart,body.fr-admin-ui .fr-db-ins { border-color:#E1E6D8; border-radius:20px; box-shadow:0 2px 10px rgba(50,109,32,.05); }
  body.fr-admin-ui .manager-dashboard-tab { justify-content:flex-start; text-align:left; min-height:67px; padding:11px 13px; }
  body.fr-admin-ui .fr-db-tab.active, body.fr-admin-ui .fr-db-apply { background:#3b791e; color:#fff; }
  body.fr-admin-ui .ma-page-title { color:#347022; font-size:20px; }
  body.fr-admin-ui .ma-page-icon { background:#347022; color:#b3a941; box-shadow:none; }
  body.fr-admin-ui .ma-reports .v-btn { border-radius:999px!important; }
  body.fr-admin-ui .fa-communications { border:1px solid #E1E6D8; border-radius:18px; overflow:hidden; }
  body.fr-admin-ui .fa-communications input::placeholder { color:rgba(255,255,255,.75); }
  body.fr-admin-ui .fa-communications .comm-card:hover { transform:translateY(-2px); box-shadow:0 6px 20px rgba(0,140,60,.12)!important; }
  body.fr-admin-ui .fr-content input,body.fr-admin-ui .fr-content select { max-width:100%; min-width:0; }
  body.fr-admin-ui .fr-content textarea { max-width:100%; }
  body.fr-admin-ui .fr-content [data-fr-grid] > * { min-width:0; }
  body.fr-admin-ui button.fr-eye-toggle,body.fr-admin-ui button.fr-eye-toggle:disabled {
    position:absolute; right:6px; top:50%; transform:translateY(-50%)!important;
    width:32px; height:32px; min-height:32px; min-width:32px; padding:0;
    border:0!important; background:transparent!important; color:#5C6B60!important;
  }
  @media(max-width:1100px) {
    body.fr-admin-ui .fr-db-kpi-grid,body.fr-admin-ui .ma-reports .v-stat-grid { grid-template-columns:repeat(2,minmax(0,1fr)); }
  }
  @media(max-width:700px) {
    body.fr-admin-ui .fr-content [data-fr-grid="responsive"] { grid-template-columns:1fr!important; }
    body.fr-admin-ui .fr-content .v-stat-grid,body.fr-admin-ui .fr-db-kpi-grid { grid-template-columns:repeat(2,minmax(0,1fr))!important; gap:10px; }
    body.fr-admin-ui .fr-db-ins-grid,body.fr-admin-ui .fr-db-bot-grid { grid-template-columns:1fr; }
    body.fr-admin-ui .manager-dashboard-tabs { grid-template-columns:1fr; }
    body.fr-admin-ui .fa-header-top,body.fr-admin-ui .ma-page-head { flex-wrap:wrap; gap:14px; }
    body.fr-admin-ui .v-section-head { align-items:flex-start; }
    body.fr-admin-ui .v-modal { padding:20px; width:100%; }
    body.fr-admin-ui .fr-content .v-kpi { padding:14px; }
    body.fr-admin-ui .v-kpi-value { font-size:22px; }
    body.fr-admin-ui .fr-db-chart { padding:16px 12px; }
    body.fr-admin-ui .fr-db-view-banner { gap:10px; flex-wrap:wrap; }
    body.fr-admin-ui .ma-report-tabs { flex-wrap:wrap; overflow:visible; }
    body.fr-admin-ui .ma-report-tab { flex:1 1 180px; justify-content:flex-start; }
    body.fr-admin-ui .fr-profile-overview { display:grid!important; grid-template-columns:56px minmax(0,1fr); gap:16px!important; padding:20px!important; }
    body.fr-admin-ui .fr-profile-overview > :first-child { width:56px!important; height:56px!important; }
    body.fr-admin-ui .fr-profile-overview > :nth-child(2) > :first-child { white-space:normal!important; overflow-wrap:anywhere; font-size:18px!important; }
    body.fr-admin-ui .fr-profile-overview > :nth-child(2) > :nth-child(2) { overflow-wrap:anywhere; font-size:11px!important; }
    body.fr-admin-ui .fr-profile-overview > :last-child { grid-column:1/-1; display:grid!important; grid-template-columns:1fr 1fr; }
    body.fr-admin-ui .fr-profile-name-fields { display:grid!important; grid-template-columns:1fr 1fr; }
    body.fr-admin-ui .fr-inventory-master-detail,body.fr-admin-ui .fr-menu-master-detail { grid-template-columns:1fr!important; min-height:0!important; max-height:none!important; }
    body.fr-admin-ui .fr-inventory-master-detail > :first-child,body.fr-admin-ui .fr-menu-master-detail > :first-child { max-height:300px!important; border-right:0!important; border-bottom:1px solid #E1E6D8; }
    body.fr-admin-ui .fr-inventory-master-detail > :last-child,body.fr-admin-ui .fr-menu-master-detail > :last-child { min-height:200px!important; max-height:none!important; }
  }
  @media(max-width:380px) {
    body.fr-admin-ui .fr-content .v-stat-grid,body.fr-admin-ui .fr-db-kpi-grid { grid-template-columns:1fr!important; }
  }
  @media(prefers-reduced-motion:reduce) {
    body.fr-admin-ui .franchisee-root * { animation:none!important; transition:none!important; }
    body.fr-admin-ui .franchisee-root [role="button"]:hover,body.fr-admin-ui .comm-card:hover { transform:none!important; }
  }
`;
...
export default function FranchiseeDashboard({ onLogout }) {
  ...
  useEffect(() => {
    ...
    const closeOnResize = () => { if (media.matches) setSidebarCollapsed(true); };
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event) => {
      if (event.key === "Escape") { setSidebarCollapsed(true); menuButtonRef.current?.focus(); }
      if (event.key !== "Tab") return;
      const controls = [...(sidebarRef.current?.querySelectorAll("button:not(:disabled)") || [])];
  ...
  const navigation = [
    ...
    { id: "menuInventory", label: "Product Catalogue", icon: <Box size={20} />, section: "main" },
    { id: "communication", label: "Announcements", icon: <MessageCircle size={20} />, section: "main" },
    { id: "staff", label: "User Management", icon: <Users size={20} />, section: "main" },
  ...
  return (
    <div className="franchisee-root">
      ...
      {/* Sidebar */}
      {!sidebarCollapsed && <div className="fr-sidebar-backdrop" onClick={() => setSidebarCollapsed(true)} aria-hidden="true" />}
      <aside id="fr-navigation" ref={sidebarRef} className="fr-sidebar" aria-label="Main navigation">
        <div className="fr-sidebar-header">
          ...
          {!sidebarCollapsed && (
            <button
              className="fr-toggle"
              type="button" aria-label="Collapse navigation"
        ...
        {sidebarCollapsed && (
          ...
          >
            <button
              className="fr-toggle"
              type="button" aria-label="Expand navigation"
      ...
      <main className="fr-main">
        <div className="fr-topbar">
          <div className="fr-topbar-heading">
            <button ref={menuButtonRef} className="fr-mobile-menu" type="button" aria-label="Open navigation" aria-expanded={!sidebarCollapsed} aria-controls="fr-navigation" onClick={() => setSidebarCollapsed(false)}><Menu size={20} /></button>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <button type="button" className="fr-avatar" aria-label="Open Profile Settings" onClick={() => setActiveModule("profile")}>{(user?.name || "F")[0]}</button>
          </div>
        </div>
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            ...
            {hasData ? (
              <>
                ...
                >
                  {[
                    { color: PAL[0], label: "Sales Revenue" },
                  ].map((l, i) => (
                    <div
                      key={i}
      ...
      <div className="manager-dashboard-tabs">
        {[
          ...
          },
          {
            id: "stock_products",
            number: "03",
            label: "Sales vs Stock Recommendations",
        ...
        >
          {dashboardTab === "overview"
            ...
            : dashboardTab === "sales_ai"
              ...
              : "Sales vs Stock Recommendations"}
        </span>
        <span style={{ height: 1, background: "#DCE9DB", flex: 1 }} />
      </div>
...
// ─── Icons ────────────────────────────────────────────────────────────────────
const SearchIcon = ({ size = 14, ...props }) => <Search size={size} {...props} />;
const XIcon = ({ size = 14, ...props }) => <X size={size} {...props} />;
const StoreIcon = ({ size = 14, color = "currentColor", ...props }) => <Store size={size} color={color} {...props} />;
const ChevronIcon = ({ size = 12, dir = "down" }) => { const Icon = dir === "up" ? ArrowUp : dir === "left" ? ChevronLeft : dir === "right" ? ChevronRight : ChevronDown; return <Icon size={size} />; };
const SortAscIcon = () => <ArrowUpAZ size={13} />;
const SortDescIcon = () => <ArrowDownAZ size={13} />;
const RefreshIcon = ({ size = 13, ...props }) => <RefreshCw size={size} {...props} />;
const LockIcon = ({ size = 13, ...props }) => <Lock size={size} {...props} />;
...
function FrStockInventoryContent({ user, brands }) {
  ...
  return (
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <ReadOnlyBanner message="Stock Inventory is view-only. Contact your admin to add, edit, or delete ingredients." />
      ...
      >
        ...
        {loading ? (
          ...
          <div data-fr-grid="responsive" className="fr-inventory-master-detail"
            style={{
              display: "grid",
      ...
      <div className="v-card" style={{ padding: "20px 22px" }}>
        <div className="v-section-head">
          <VSectionTitle icon={<Users size={16} />}>
            User Management — {franchiseeBranch}
          </VSectionTitle>
          <button
            className="v-btn v-btn-primary"
            onClick={() => {
    ...
    <div className="fa-communications" style={commStyles.root}>
      ...
      <div style={commStyles.header}>
        ...
        <div className="fa-header-top" style={commStyles.headerTop}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <button type="button" aria-label="Refresh announcements" title="Refresh announcements" disabled={fetching} onClick={fetchAnnouncements} style={{ ...commStyles.liveChip, color: "white", cursor: "pointer" }}><RefreshCw size={14} /></button>
            <button
              type="button" aria-label="Toggle announcement search" aria-expanded={searchVisible}
              onClick={() => {
          ...
          filtered.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className="comm-card"
                role="button" tabIndex={0} aria-label={`Read announcement: ${item.title}`}
                onKeyDown={(event) => {
                  if (event.target !== event.currentTarget) return;
                  if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setViewingItem(item); }
                }}
...
function FrProfileContent({ user }) {
  ...
  const EyeToggle = ({ show, onToggle, disabled }) => (
    <button type="button" className="fr-eye-toggle" aria-label={show ? "Hide password" : "Show password"} aria-pressed={show} onClick={onToggle} disabled={disabled}>
  ...
  return (
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      >
        ...
        <div className="fr-profile-overview"
          style={{
            padding: "22px 24px",
            flexWrap: "wrap",
            display: "flex",
      ...
      >
        ...
        >
          ...
          <form onSubmit={handleSubmit} style={{ padding: "22px 24px" }}>
            {/* Name */}
            <div className="fr-profile-name-fields" style={{ display: "flex", gap: 10, marginBottom: 14 }}>
              <div style={{ flex: 2 }}>
