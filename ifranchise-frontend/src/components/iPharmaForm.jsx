...
const auditLog = (() => {
  ...
  const getSession = () =>
    ...
    (() => {
      const id = `IP-SES-${Date.now()}`;
      sessionStorage.setItem("ip_session", id);
      return id;
    })();
  return {
    record: (action, data = {}) => {
    ...
    <div style={S.overlay}>
      <div style={S.modalBox}>
        <div
          style={{
            ...S.iconWrap,
            background: ok ? COLOR.tintGreen : "#fdecea",
          }}
        >
          {ok ? (
  ...
  return (
    <div style={S.overlay}>
      ...
      >
        ...
        >
          <p>
            <strong>1. APPLICATION PROCESS</strong>
            <br />
            By submitting this iPharma Mart Franchise Application Form, you
            acknowledge this is an expression of interest only and does not
            constitute a binding agreement.
          </p>
          <p>
            <strong>2. DATA PRIVACY (RA 10173)</strong>
            <br />
            All personal data collected is governed by the Data Privacy Act of
            2012. Your information will be used solely for evaluating your
            franchise application.
          </p>
          <p>
            <strong>3. NON-TRANSFERABILITY</strong>
            <br />
            The franchise application and, if approved, the franchise agreement
            are personal to the applicant and strictly non-transferable.
          </p>
          <p>
            <strong>4. ID VALIDATION</strong>
            <br />
            You consent to OCR technology verifying your government-issued ID.
            Data extracted will be used solely for identity verification.
          </p>
          <p>
            <strong>5. AUDIT TRAIL</strong>
            <br />
            All actions performed during this application are logged with
            timestamps and session identifiers as part of our tamper-evident
            digital audit trail.
          </p>
          <p>
            <strong>6. OTP VERIFICATION</strong>
            <br />A One-Time Password will be sent to your registered mobile
            number. Standard SMS rates may apply.
          </p>
          <p>
            <strong>7. NO GUARANTEE</strong>
            <br />
            Submission does not guarantee approval. iFranchise reserves the
            right to approve or reject any application at its sole discretion.
          </p>
          <p>
            <strong>8. GOVERNING LAW</strong>
            <br />
            This application shall be governed by the laws of the Republic of
            the Philippines.
          </p>
        ...
        <button
          style={{ ...S.btn, ...S.btnSolid, marginTop: 16, width: "100%" }}
          onClick={onClose}
        >
...
function OtpModal({
  ...
  const verify = () => {
    ...
    if (code === expectedOtp) {
      ...
      if (n >= maxAttempts) {
        setError(`Maximum ${maxAttempts} attempts reached.`);
        setCanResend(true);
      } else {
        setError(`Incorrect OTP. ${maxAttempts - n} attempt(s) remaining.`);
      }
      setOtp(["", "", "", "", "", ""]);
    ...
    <div style={S.overlay}>
      <div style={{ ...S.modalBox, width: 380 }}>
        <div style={{ ...S.iconWrap, background: COLOR.tintGreen }}>
          <Smartphone size={38} color={COLOR.green900} strokeWidth={2} />
        </div>
        <h3
          style={{
            margin: "0 0 6px",
        ...
        >
          ...
          <strong>
            {mobile?.replace(/(\d{4})(\d{3})(\d{4})/, "$1-$2-$3")}
          </strong>
          .
          <br />
        </p>
        ...
        <div style={{ overflowY: "auto", flex: 1 }}>
          ...
          {step === "front" && (
            <div>
              ...
              >
                Ensure good lighting. All text must be clearly visible.
...
export default function IPharmaForm() {
  ...
  const handleChange = (e) => {
    ...
    if (["equity", "investment", "dependents"].includes(name))
    ...
    if (
      ["involvement", "otherBusiness", "familyDepend", "location"].includes(
        name,
      )
    )
  ...
  const validateField = (name, value) => {
    ...
    if (optional.includes(name)) {
      if (name === "altMobile" && value && !/^09\d{9}$/.test(value))
        return "Please enter valid Philippine number (09**********)";
      return "";
    }
    ...
    if (name === "dob") {
      const age = Math.floor(
        (new Date() - new Date(value)) / (365.25 * 24 * 3600 * 1000),
      );
      if (age < 18) return "Must be at least 18 years old";
    }
    if (name === "equity" && (Number(value) <= 0 || Number(value) > 100))
      return "Enter a value between 1 and 100";
    if (name === "investment" && Number(value) <= 0)
  ...
  const addEduc = () =>
    setEducList((p) => [
      ...p,
      { degree: "", school: "", yearGrad: "", course: "" },
    ]);
  const updateEduc = (i, field, val) =>
    setEducList((p) => {
      const n = [...p];
      n[i] = { ...n[i], [field]: val };
      return n;
  ...
  const validatePage = (fields, extras = []) => {
    ...
    if (extras.includes("maritalStatus") && !maritalStatus)
      e.maritalStatus = "Please select marital status";
    if (extras.includes("addrRegion") && !addrRegion)
      e.addrRegion = "Please select a region";
    if (
      extras.includes("addrProvince") &&
      provinces.length > 0 &&
      !addrProvince
    )
      e.addrProvince = "Please select a province";
    if (extras.includes("addrCity") && !addrCity)
      e.addrCity = "Please select a city/municipality";
    if (extras.includes("addrBarangay") && !addrBarangay)
      e.addrBarangay = "Please select a barangay";
    if (extras.includes("addrStreet") && !addrStreet)
      e.addrStreet = "Please enter a street address";
    if (extras.includes("idVerified") && !idVerified)
      e.idVerified = "Please complete ID verification";
    if (extras.includes("letterOfIntent") && !letterOfIntent)
      e.letterOfIntent = "Please upload your Letter of Intent";
    if (extras.includes("terms") && !termsAccepted)
      e.terms = "You must accept the Terms and Conditions";
    if (extras.includes("consent") && !consentAccepted)
      e.consent = "You must give your data privacy consent";
    setErrors((p) => ({ ...p, ...e }));
  ...
  const handleFinalSubmit = async () => {
    ...
    if (!validatePage(p3Fields, p3Extras)) {
      ...
      showAlert(
        "error",
        "Please fill in all required fields and complete all verification steps.",
      );
      return;
    }
    ...
    if (isDuplicate) {
      ...
      showAlert(
        "error",
        "An application with this email or mobile number already exists.",
      );
      setLoading(null);
      return;
    }
  ...
  const handleOtpVerified = async (success) => {
    ...
    if (letterOfIntent) {
      try {
        ...
        showAlert(
          "error",
          "Failed to process Letter of Intent. Please try again.",
        );
        return;
      }
    }
    ...
    const payload = {
      ...
      familyDepend: form.familyDepend,
      marketArea: form.marketArea,
    ...
    try {
      const res = await fetch(
        `${process.env.REACT_APP_API_URL}/ipharma-applications`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (data.success) {
        ...
        showAlert(
          "success",
          "iPharma Mart Application submitted successfully! We will review your application and contact you soon.",
          () => {
            closeAlert();
            navigate("/");
          },
    ...
    <div className="ip-page">
          ...
          <div className="ip-progress">
            <div className="ip-steps">
              {steps.map((st) => (
                <span
                  key={st.label}
                  className={`ip-step ${progress >= st.pct ? "active" : ""}`}
                >
                  {st.label}
                </span>
              ))}
      ...
      <div className="ip-content">
        ...
        <div className="ip-card">
          ...
          <p className="ip-sub">
            Complete the form below to begin your journey with iPharma Mart
          </p>
          ...
          {page === 1 && (
            ...
            >
                ...
                {!idVerified ? (
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        Personal details have been auto-filled from your ID
                        scan.
                      </p>
                    </div>
                    <button
              ...
              <div className="ip-section">
                ...
                {educList.length === 0 && (
                  <p style={{ fontSize: 12, color: COLOR.muted, margin: "0" }}>
                    No education records added yet. Click below to begin.
                  </p>
                )}
                {educList.map((educ, i) => (
                  <div
                  ...
                  >
                    ...
                    <div className="ip-row">
                      <Field label="Degree / Level" required half>
                        <CustomSelect
                          ...
                          onSelect={(v) => updateEduc(i, "degree", v)}
                        />
                      </Field>
                      <Field label="Year Graduated" half>
                        <input
                          ...
                          value={educ.yearGrad}
                          onChange={(e) =>
                            updateEduc(i, "yearGrad", e.target.value)
                          }
                        />
                      </Field>
                    </div>
                    <Field label="School / University">
                      <input
                        ...
                        value={educ.school}
                        onChange={(e) =>
                          updateEduc(i, "school", capitalize(e.target.value))
                        }
                      />
                    </Field>
                    <Field label="Course / Program">
                      <input
                        ...
                        placeholder="e.g. BS Pharmacy"
                        value={educ.course}
                        onChange={(e) =>
                          updateEduc(i, "course", capitalize(e.target.value))
                        }
                      />
                    </Field>
                  </div>
              ...
              <button
                ...
                onClick={() => {
                  if (validatePage(p1Fields, p1Extras)) {
                    auditLog.record("PAGE_1_COMPLETE");
                    setPage(2);
                    window.scrollTo(0, 0);
                  }
                }}
              >
                Next: Business Interest{" "}
            ...
            >
              <div className="ip-section">
                ...
                >
                  <textarea
                    ...
                    name="otherBusiness"
                    placeholder="List any other business interests or ventures (optional)"
              ...
              <div style={{ display: "flex", gap: "1rem" }}>
                ...
                <button
                  className="ip-submit-btn"
                  onClick={() => {
                    if (validatePage(p2Fields)) {
                      auditLog.record("PAGE_2_COMPLETE");
                      setPage(3);
                      window.scrollTo(0, 0);
                    }
                  }}
          ...
          {page === 3 && (
            ...
            >
              <div className="ip-section">
                ...
                <Field
                  label="Family Dependence on Franchise Income"
                  required
                  error={errors.familyDepend}
                >
                  <textarea
                    style={{
                      ...inpStyle,
                ...
                <Field
                  label="Immediate Market Area"
                  required
                  error={errors.marketArea}
                >
                  {inp(
                    "marketArea",
                    "Describe the immediate market area for your franchise",
                  )}
                </Field>
                <Field
              ...
              <div className="ip-section">
                ...
                >
                  ...
                  >
                    ...
                    <strong>Data Privacy Act of 2012 (RA 10173)</strong> for the
                    purpose of evaluating my franchise application.{" "}
                    <span style={{ color: "#EF4444" }}>*</span>
                  </span>
                </label>
                {errors.consent && (
                ...
                >
                  ...
                  <span style={{ fontSize: 12, color: "#e65100" }}>
                    ...
                    non-transferable. The name verified by your
                    government-issued ID will be the sole authorized franchisee.
                  </span>
                </div>
              </div>
              ...
              <div style={{ display: "flex", gap: "1rem" }}>
                ...
                <button className="ip-submit-btn" onClick={handleFinalSubmit}>
                  Submit Application & Verify via OTP{" "}
                  <ArrowRight size={16} style={{ marginLeft: 8 }} />
                </button>
              </div>
              <p className="ip-footer-note">
                By submitting, you agree to our terms. An OTP will be sent to
