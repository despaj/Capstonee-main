...
import {
  Bell, Home, Box, FileText, FileCheck, Users, BarChart2, MessageCircle,
  ...
  AlertCircle, LoaderCircle, UploadCloud, ArrowLeft, ArrowRight, Truck, Filter, ChevronUp, Tag, Link2, PackageCheck
} from 'lucide-react';
...
const VEmptyState = ({ icon, title, sub }) => {
  ...
  <div className="v-empty">
    <div className="v-empty-icon" style={{display:'flex',justifyContent:'center',color:'#3b791e'}}>{renderedIcon}</div>
...
const ReadOnlyBanner = ({ message = 'View only — contact your admin to make changes.' }) => (
  <div style={{
    ...
    fontSize: 12, fontWeight: 600, color: '#2563eb', fontFamily: 'Plus Jakarta Sans,sans-serif',
  }}>
...
export default function ManagerDashboard() {
  ...
  useEffect(() => {
    ...
    const brand = String(user?.brand || user?.brand_name || user?.brandName || '').trim();
    ...
    const refresh = async () => {
      ...
      try {
        ...
        const [stockResponse, notificationResponse] = await Promise.all([
          fetch(`${process.env.REACT_APP_API_URL}/ingredients?${params}`, options),
          fetch(`${process.env.REACT_APP_API_URL}/notifications?${new URLSearchParams({ userId:String(user.id) })}`, options),
        ]);
        if (!stockResponse.ok || !notificationResponse.ok) throw new Error('Notification request failed');
        const [stock, recorded] = await Promise.all([stockResponse.json(), notificationResponse.json()]);
        if (!Array.isArray(stock) || !Array.isArray(recorded)) throw new Error('Unexpected notification response');
        const low = stock.filter(item =>
          normalize(item.brand) === normalize(brand) && normalize(item.branch) === normalize(branch) &&
          item.stock !== null && item.stock !== undefined && item.min_stock !== null && item.min_stock !== undefined &&
        ...
        const items = low.length ? [{
          id:'manager-low-stock', module:'stockInventory', title:`${brand} — Low Stock`,
          message:`${branch}: ${low.slice(0,3).map(item => item.name).join(', ')}${low.length > 3 ? ` and ${low.length-3} more` : ''}. Open Stock Inventory to review quantities.`,
          count:low.length, icon:AlertTriangle, bg:'#fff7ed', color:'#3b791e', border:'#c9dba0',
        }] : [];
        recorded.filter(n => String(n.user_id) === String(user.id) && !n.is_read && n.type !== 'low_stock' &&
          (!n.brand || normalize(n.brand) === normalize(brand)) && (!n.branch || normalize(n.branch) === normalize(branch))
        ).forEach(n => items.push({
          id:`manager-record-${n.id}`, recordId:n.id,
          title:n.title || 'Branch notification', message:n.body || '', count:1,
          module:n.type === 'announcement' ? 'communication' : String(n.type || '').includes('report') ? 'reports' : 'dashboard',
          icon:Bell, bg:'#f0f5e8', color:'#3b791e', border:'#c9dba0',
        }));
        if (!disposed) { setManagerNotifications(items); setManagerNotifError(''); }
      } catch (error) {
        if (!disposed && error.name !== 'AbortError') setManagerNotifError('Unable to refresh notifications. Retry using the bell refresh button.');
      } finally {
        pending = false;
  ...
  const scopedBrands = useMemo(() => {
    const accountBrand = String(user?.brand || user?.brand_name || user?.brandName || '').trim().toLowerCase();
    if (accountBrand) return brands.filter(b => String(b.name || '').trim().toLowerCase() === accountBrand);
    const accountBranch = String(user?.branch || '').trim().toLowerCase();
    if (!accountBranch) return [];
    return brands.filter(b => (b.branches || []).some(br =>
      String(typeof br === 'string' ? br : br?.name || '').trim().toLowerCase() === accountBranch
    ));
  }, [brands, user]);
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/transactions?${params}`, { credentials:'include' })
      .then(r => r.ok ? r.json() : Promise.reject(new Error('Transaction request failed')))
    ...
    <div className="franchisee-root manager-dashboard-root">
      ...
      <aside className="fr-sidebar">
        <div className="fr-sidebar-header">
          ...
          {sidebarCollapsed && <img src={logoIfranchise} alt="iFranchise" style={{ height: 35, width: 'auto', objectFit: 'contain', margin: '10px auto', display: 'block' }} />}
          {!sidebarCollapsed && (
      ...
      <main className="fr-main">
        <div className="fr-topbar">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div title={managerNotifError || 'Branch notifications'}>
              <NotificationBell key={`${user?.id}:${user?.brand || user?.brand_name || user?.brandName}:${user?.branch}`}
                notifications={managerNotifications} loading={managerNotifLoading}
                onRefresh={() => managerNotifRefresh.current()}
                onNavigate={notification => {
                  setActiveModule(notification.module);
                  if (notification.recordId != null) {
                    fetch(`${process.env.REACT_APP_API_URL}/notifications/${encodeURIComponent(notification.recordId)}/read`, {
                      method:'PATCH', credentials:'include',
                    }).then(response => { if (!response.ok) throw new Error('Unable to mark notification read'); })
                      .catch(() => setManagerNotifError('Read status could not be saved. Please refresh notifications.'));
                  }
                }} />
              {managerNotifError && <span role="status" style={{ display:'block', maxWidth:180, fontSize:10, color:'#b45309' }}>{managerNotifError}</span>}
            </div>
        ...
        <div className="fr-content">
          {activeModule === 'dashboard'      && <BranchDecisionDashboard transactions={transactions} brands={scopedBrands} user={user} />}
          {activeModule === 'menuInventory'  && <MenuInventoryContent  user={user} brands={scopedBrands} />}
          {activeModule === 'stockInventory' && <StockInventoryContent user={user} brands={scopedBrands} />}
      ...
      {showLogoutModal && (
        <div className="v-modal-overlay" style={{ zIndex: 3000 }} onClick={() => setShowLogoutModal(false)}>
          <div className="v-modal" style={{ maxWidth: 400, textAlign: 'center' }} onClick={e => e.stopPropagation()}>
            <div style={{ width: 68, height: 68, borderRadius: '20px', background: 'linear-gradient(135deg,rgba(239,68,68,0.12),rgba(220,38,38,0.08))', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.5rem', color:'#dc2626', border: '1.5px solid rgba(239,68,68,0.15)' }}><LogOut size={30} /></div>
            <h2 className="v-modal-title" style={{ textAlign: 'center' }}>Log out?</h2>
            <p style={{ color: '#9CA89C', fontSize: 13, margin: '8px 0 24px', lineHeight: 1.6, fontFamily: 'Plus Jakarta Sans,sans-serif' }}>You'll need to sign in again to access your account.</p>
...
function ProductAnalyticsPanel({ preset, appliedRange, rangeMode, filterBranch, filterBrand, selectedBrand }) {
  ...
  const BAR_COLORS = ['#509820','#3b791e','#26a69a','#43a047','#66bb6a','#80cbc4','#a5d6a7','#D4DBC8','#c8e6c9','#E1E6D8'];
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid rgba(59,121,30,0.12)', borderRadius: 22, padding: '22px 24px', boxShadow: '0 2px 20px rgba(50,109,32,0.07)', marginTop: 24 }}>
      ...
      {!loading && data && (tab === 'top10' || tab === 'fast' || tab === 'slow') && (() => {
        ...
        return (
          <div>
            {/* Column headers */}
            <div style={{ display: 'grid', gridTemplateColumns: '24px 1fr 90px 90px 180px', gap: 8, padding: '6px 10px', borderBottom: '2px solid #E1E6D8', fontSize: 10, fontWeight: 800, color: '#3b791e', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 4 }}>
            ...
            {list.map((p, i) => (
              <div key={p.name}
                ...
                <span style={{ fontSize: 11, fontWeight: 800, color: i < 3 ? ['#f59e0b','#9CA89C','#cd7c2e'][i] : '#9ca3af' }}>
                  {i < 3 ? ['1','2','3'][i] : `${i+1}`}
                </span>
                <div>
    ...
    <div style={{
      ...
      {!analysis && !loading && !error && (
        <div style={{
          ...
          <div style={{ display:'flex', justifyContent:'center', color:'#185FA5', marginBottom: 8 }}><Brain size={28} /></div>
...
function PanelCard({ children, style: s }) {
  return (
    <div style={{ background: "#fff", border: "1px solid rgba(59,121,30,0.12)", borderRadius: 18, overflow: "hidden", boxShadow: "0 2px 16px rgba(50,109,32,0.07)", ...s }}>
      {children}
    </div>
  );
}

function CardHeader({ icon: Icon, title, sub, gradient = "linear-gradient(135deg,#3b791e,#3b791e)", action }) {
  return (
...
function SalesTrendSection({ values, labels, kpiData, total, avg, peak, low, peakLabel, pctChange, trending, getRangeLabel, filterLabel }) {
  ...
  const branchData = useMemo(() => {
    return Array.isArray(kpiData?.branchBreakdown) ? kpiData.branchBreakdown.slice(0, 5) : [];
  }, [kpiData]);

  const previousValues = Array.isArray(kpiData?.previousPeriodValues) ? kpiData.previousPeriodValues : [];
  const gpLine = Array.isArray(kpiData?.grossProfitTrend) ? kpiData.grossProfitTrend : [];
  ...
  const analysisBullets = useMemo(() => {
    ...
    if (grossProfit !== null) bullets.push(`Gross profit stands at ${fmtAmt(grossProfit)}, a ${Math.round((grossProfit / (kpiData?.totalSales ?? total)) * 100)}% margin.`);
    if (txCount !== null) bullets.push(`${Number(txCount).toLocaleString()} transactions processed${avgOrder !== null ? ` with an average order of ${fmtAmt(avgOrder)}` : ''}.`);
      ...
      <div style={{ padding: "18px 20px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 18, marginBottom: 14, alignItems: "stretch" }}>
          <div style={{ display: "flex", flexDirection: "column" }}>
            ...
            {hasData ? (
              <>
                <ComboChart barData={previousValues.length === values.length ? [values, previousValues] : [values]} lineData={gpLine} labels={labels} height={220} />
                <div style={{ display: "flex", gap: 16, marginTop: 10, marginBottom: 14, flexWrap: "wrap" }}>
                  {[
                    { color: PAL[0], label: "Sales CY" },
                    ...(previousValues.length === values.length ? [{ color: PAL[1], label: "Previous Period" }] : []),
                    ...(gpLine.length === values.length ? [{ color: "#1d4ed8", label: "Gross Profit %", line: true }] : []),
                  ].map((l, i) => (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8, flex: 1 }}>
                  {[
                    ...
                    ...(grossProfit !== null ? [{ label: "Gross Profit", text: `Gross profit stands at ${fmtAmt(grossProfit)}, a ${Math.round((grossProfit / (kpiData?.totalSales ?? (total || 1))) * 100)}% margin.`, icon: BarChart2, color: "#1d4ed8", bg: "#eff6ff", border: "#bfdbfe" }] : []),
              ...
              <div style={{ flex: 1, minHeight: 220, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "#fbfdf6", borderRadius: 12, border: "1.5px dashed #D4DBC8" }}>
                ...
                <div style={{ fontSize: 11, color: "#9CA89C", marginTop: 4, fontFamily: FONT }}>Try a different range, brand, or branch</div>
              </div>
            )}
          </div>

          <div style={{ background: "linear-gradient(160deg,#F6F7F1,#eaf5ec)", border: "1px solid #c8e6c9", borderRadius: 14, padding: "16px 14px", display: "flex", flexDirection: "column" }}>
            ...
            {hasData ? (
              ...
              <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8 }}>
                ...
                <p style={{ fontSize: 11.5, color: "#9CA89C", textAlign: "center", lineHeight: 1.6, margin: 0, fontFamily: FONT }}>Select a date range and branch to see analysis.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
          <div style={{ background: "#fbfdf6", border: "1px solid #E1E6D8", borderRadius: 14, padding: "14px 16px" }}>
    ...
    <PanelCard style={{ marginBottom: 22 }}>
      ...
      <div style={{ padding: "18px 20px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 20 }}>
          {[
            { label: "Projected 7-Day Revenue", value: projRev !== null ? fmtAmt(projRev) : "—", sub: projRev !== null && projChg !== null ? `${projChg >= 0 ? "+" : ""}${Number(projChg).toFixed(1)}% vs prior` : "Run AI to populate", color: "#059669", bg: "#ecfdf5", border: "#a7f3d0", icon: TrendingUp },
              ...
              {analysis ? (
                ...
                <>
                  {preRunBullets.length > 0
                    ...
                    : <p style={{ fontSize: 12, color: "#9CA89C", fontFamily: FONT, fontStyle: "italic" }}>Load transactions and run AI Analysis to generate insights.</p>
                  }
                  {error && (
              ...
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {analysis.stockAnomalies.map((a, i) => {
                    ...
                    }[a.anomalyType] || { bg: "#fbfdf6", border: "#E1E6D8", label: "Anomaly", labelBg: "#E1E6D8", labelColor: "#2c5c16", dot: "#3b791e" };
                    return (
            ...
            {analysis?.recommendations?.length > 0 ? (
              <>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                  ...
                  <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 20, background: "#E1E6D8", color: "#2c5c16", fontFamily: FONT }}>{analysis.recommendations.length}</span>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {analysis.recommendations.map((rec, i) => {
                    const s = typeStyle(rec.type);
              ...
              <div style={{ background: "#fafbff", border: "1.5px dashed #dbeafe", borderRadius: 14, padding: "28px 20px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", gap: 10 }}>
                ...
                <p style={{ fontSize: 11.5, color: "#9CA89C", textAlign: "center", lineHeight: 1.65, margin: 0, fontFamily: FONT }}>
                  {transactions?.length
...
function SalesVsStockSection({ preset, appliedRange, rangeMode, filterBranch, filterBrand, selectedBrand, total }) {
  ...
  const moverPie = totalSKUs > 0 ? [
    ...
    { label: "Normal",      value: Math.max(0, totalSKUs - fastCount - slowCount), color: "#9CA89C" },
  ].filter(d => d.value > 0) : [];
      ...
      <div key={p.name}
        ...
        <div style={{ minWidth: 0 }}>
          ...
          {!isBuyers && p.branchBreakdown && (
            <div style={{ fontSize: 9.5, color: "#9CA89C", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontFamily: FONT }}>
              {Object.entries(p.branchBreakdown).slice(0, 2).map(([br, q]) => `${br}: ${q}`).join(" · ")}
            </div>
          )}
        </div>
        ...
        <div style={{ paddingLeft: 8 }}>
          {isBuyers
            ? <span style={{ background: "#E1E6D8", color: "#2c5c16", padding: "2px 8px", borderRadius: 20, fontSize: 10, fontWeight: 700, display: "inline-block", maxWidth: "100%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontFamily: FONT }}>{p.topProduct}</span>
            : <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <span style={{ fontSize: 9.5, fontWeight: 700, color: "#9CA89C", minWidth: 28, textAlign: "right", fontFamily: FONT }}>{Math.round((p.totalRevenue / maxR) * 100)}%</span>
              </div>
          }
        </div>
      </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 18 }}>
          <div>
            ...
            {!loading && (top10.length > 0 || fast.length > 0 || slow.length > 0 || data?.topBuyers?.length > 0) && (
              <div style={{ display: "grid", gridTemplateColumns: tab === "buyers" ? "28px 1fr 70px 1fr" : "28px 1fr 65px 70px 1fr", gap: 8, padding: "7px 10px", borderBottom: "2px solid #e8f5e9", fontSize: 9.5, fontWeight: 800, color: "#3b791e", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 4, fontFamily: FONT }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ background: "#fbfdf6", border: "1px solid #E1E6D8", borderRadius: 14, padding: "13px 14px" }}>
...
function BranchOperationsSnapshot({ transactions = [], preset, rangeMode, appliedRange }) {
  const rows = useMemo(() => {
    const now = new Date();
  ...
  const metrics = useMemo(() => {
    ...
    rows.forEach(tx => {
      ...
      const payment = tx.payment_method || tx.paymentMethod || tx.payment_type || 'Unspecified';
      payments.set(payment, (payments.get(payment) || 0) + Number(tx.total || 0));
      ...
      (Array.isArray(items) ? items : []).forEach(item => {
        const name = item.product_name || item.productName || item.name || item.menu_name || 'Unnamed Product';
    ...
    return {
      ...
      slow: rankedProducts.length > 1 ? rankedProducts[rankedProducts.length - 1] : null,
      peakHour,
      paymentRows,
    };
  }, [rows]);
  ...
  const cards = [
    { label: 'Transactions', value: rows.length.toLocaleString(), sub: `${metrics.units.toLocaleString()} units sold`, icon: Receipt, color: '#3b791e', bg: '#f0f5e8' },
    { label: 'Best Seller', value: metrics.top?.[0] || 'No sales data', sub: metrics.top ? `${metrics.top[1]} units sold` : 'Record product-level items', icon: TrendingUp, color: '#3b791e', bg: '#f0f5e8' },
    { label: 'Needs Attention', value: metrics.slow?.[0] || 'Not enough data', sub: metrics.slow ? `${metrics.slow[1]} units sold` : 'Requires at least two products', icon: TrendingDown, color: '#b45309', bg: '#fff7ed' },
    { label: 'Busiest Hour', value: hourLabel(metrics.peakHour?.[0]), sub: metrics.peakHour ? `${fmtPeso(metrics.peakHour[1])} sales` : 'No transactions yet', icon: Clock, color: '#1d4ed8', bg: '#eff6ff' },
  ];

  return (
    <section style={{ background: '#fff', border: '1px solid #E1E6D8', borderRadius: 16, padding: '20px 22px', boxShadow: '0 8px 24px rgba(50,109,32,.06)', marginBottom: 18 }}>
      <div className="v-section-head">
        <VSectionTitle icon={<Activity size={18} />}>Branch Operations Snapshot</VSectionTitle>
        <span style={{ fontSize: 11.5, color: '#5C6B60', fontWeight: 600 }}>Selected period · {fmtPeso(metrics.revenue)} revenue</span>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(190px,1fr))', gap: 12 }}>
        {cards.map(({ label, value, sub, icon: Icon, color, bg }) => (
          <div key={label} style={{ border: '1px solid #E1E6D8', borderRadius: 14, padding: '15px 16px', minWidth: 0 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, marginBottom: 9 }}>
              <div style={{ minWidth: 0 }}>
                <div className="v-kpi-label">{label}</div>
                <div style={{ fontSize: 16, fontWeight: 800, color: '#12241B', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={String(value)}>{value}</div>
              </div>
              <div style={{ width: 36, height: 36, borderRadius: 11, background: bg, color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Icon size={17} /></div>
            </div>
            <div className="v-kpi-sub">{sub}</div>
          </div>
        ))}
      </div>
      <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid #E1E6D8' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 9, fontSize: 12, fontWeight: 800, color: '#12241B' }}><CreditCard size={14} color="#3b791e" /> Payment Mix</div>
        {metrics.paymentRows.length === 0 ? (
          <div style={{ color: '#9CA89C', fontSize: 12 }}>No payment data for this period.</div>
        ) : (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {metrics.paymentRows.map(([method, amount]) => {
              const share = metrics.revenue > 0 ? (amount / metrics.revenue) * 100 : 0;
              return <span key={method} style={{ padding: '7px 11px', borderRadius: 999, background: '#F6F7F1', border: '1px solid #E1E6D8', fontSize: 11.5, color: '#374132' }}><strong>{method}</strong> · {share.toFixed(0)}% ({fmtPeso(amount)})</span>;
            })}
          </div>
        )}
      </div>
...
const safeDate = value => { const d = new Date(value || 0); return Number.isNaN(d.getTime()) ? null : d; };
const numberOrNull = value => value === '' || value === null || value === undefined || Number.isNaN(Number(value)) ? null : Number(value);
const evidenceId = (row, index = 0) => firstValue(row, ['transaction_id','reference_no','reference_id','receipt_no','batch_no','id'], `Record ${index + 1}`);
...
const transactionCogsAvailable = (transactions = []) =>
  transactions.length > 0 && transactions.every(transaction =>
...
function useCountUp(value, duration = 650) {
  ...
  useEffect(() => {
    ...
    if (from === to || typeof to !== "number") { setDisplay(to); fromRef.current = to; return; }
...
function BdTabs({ tabs, active, onChange }) {
  ...
  return (
    <div className="bd-tabs-wrap">
      <div className="bd-tab-indicator" style={{ left: ind.left, width: ind.width }} />
      {tabs.map(t => (
...
function BdKpiCard({ icon: Icon, tone = "neutral", label, value, sub, trend, onClick }) {
  const toneMap = {
  ...
  return (
    <button className="bd-kpi" onClick={onClick}>
      <div className="bd-kpi-top">
        <span className="bd-kpi-icon" style={{ background: toneMap.bg, color: toneMap.color }}><Icon size={16} /></span>
        {trend != null && (
      ...
      <div className="bd-kpi-value">{isNumber ? Math.round(animated).toLocaleString("en-PH") : value}</div>
...
function BdMiniCard({ icon: Icon, label, value, sub, onClick }) {
  return (
    <button className="bd-mini-card" onClick={onClick} disabled={!onClick}>
      <span className="bd-mini-icon"><Icon size={14} /></span>
...
function BdTrendChart({ points, maxGraph, onPointClick }) {
  ...
  points.forEach((p, i) => {
    ...
    else { const prev = points[i - 1]; const cx = (prev.x + p.x) / 2; path += ` C ${cx} ${prev.y}, ${cx} ${p.y}, ${p.x} ${p.y}`; }
  });
  const area = points.length ? path + ` L ${points[points.length - 1].x} 146 L ${points[0].x} 146 Z` : "";
  ...
  const handleMove = (e) => {
    ...
    points.forEach(p => { const d = Math.abs(p.x - mx); if (d < bd) { bd = d; best = p; } });
    setTip(best);
  };

  const yTicks = [0, 0.5, 1].map(f => ({ y: 16 + 130 * (1 - f), label: fmtShort(maxGraph * f) }));
  const showEveryLabel = points.length <= 15;

  return (
    <div style={{ position: "relative" }} onMouseMove={handleMove} onMouseLeave={() => setTip(null)}>
      <svg viewBox="0 0 800 178" style={{ width: "100%", maxHeight: 190, display: "block", overflow: "visible" }}>
        <defs>
        ...
        {yTicks.map((t, i) => (
          <g key={i}>
            <line x1="55" y1={t.y} x2="775" y2={t.y} stroke={C.border} strokeDasharray="3 5" />
            <text x="46" y={t.y + 3} textAnchor="end" fontSize="9" fill={C.muted}>{t.label}</text>
          </g>
        ))}
        {area && <path d={area} fill="url(#bddArea)" opacity={drawn ? 1 : 0} style={{ transition: "opacity .6s ease .2s" }} />}
        {path && (
          <path ref={pathRef} d={path} fill="none" stroke={C.green} strokeWidth="2.25" strokeLinecap="round"
            style={{ strokeDasharray: len, strokeDashoffset: drawn ? 0 : len, transition: "stroke-dashoffset 1s cubic-bezier(.3,.7,.3,1)" }} />
        )}
        {points.map((p, i) => (
          <circle key={i} cx={p.x} cy={p.y} r={tip?.label === p.label ? 4.5 : 2.5}
            fill={C.green} stroke={C.white} strokeWidth="1.75" style={{ cursor: "pointer", transition: "r .12s" }}
            onClick={() => onPointClick(p)} />
        ))}
        {tip && <line x1={tip.x} y1="16" x2={tip.x} y2="146" stroke={C.greenMid} strokeDasharray="3 4" />}
        {points.map((p, i) => (showEveryLabel || i % Math.ceil(points.length / 12) === 0) && (
          <text key={p.label} x={p.x} y="166" textAnchor="middle" fontSize="9" fill={C.muted}>{p.label}</text>
        ))}
      </svg>
      {tip && (
        <div className="bd-chart-tooltip" style={{ left: `${(tip.x / 800) * 100}%`, top: `${(tip.y / 178) * 100}%` }}>
...
function BranchDecisionDashboard({ transactions = [], brands = [], user }) {
  const branch = String(user?.branch || '').trim();
  const brand = String(user?.brand || user?.brand_name || user?.brandName || brands?.[0]?.name || '').trim();
  ...
  const [sources,setSources] = useState({ inventory:[], movements:[], loading:true, errors:[] });
  ...
  const scope = useCallback(row => {
    const rowBranch = normText(firstValue(row,['branch','branch_name','location']));
    const rowBrand = normText(firstValue(row,['brand','brand_name']));
    return (!rowBranch || rowBranch === normText(branch)) && (!rowBrand || !brand || rowBrand === normText(brand));
  },[branch,brand]);

  useEffect(() => {
    if (!branch) { setSources({inventory:[],movements:[],loading:false,errors:['Authenticated account has no assigned branch.']}); return; }
    let live = true;
    const params = new URLSearchParams({branch}); if (brand) params.set('brand',brand);
    const endpoints = [['inventory','/ingredients'], ['menu','/inventory']];
    Promise.all(endpoints.map(async ([name,path]) => {
      try {
        const res = await fetch(`${api}${path}?${params}`,{credentials:'include'});
        if (!res.ok) throw new Error(`${res.status}`);
        const json = await res.json();
        const rows = Array.isArray(json) ? json : firstValue(json,[name,'items','records','data'],[]);
        return {name,rows:Array.isArray(rows) ? rows.filter(scope) : [],ok:true};
      ...
      const batchResults = await Promise.all(ingredients.map(async ingredient => {
        try {
          const response = await fetch(`${api}/ingredient-batches?ingredient_id=${ingredient.id}`,{credentials:'include'});
          if (!response.ok) throw new Error(`${response.status}`);
          const rows = await response.json();
          return {ingredient_id:ingredient.id,rows:Array.isArray(rows)?rows:[],ok:true};
        } catch { return {ingredient_id:ingredient.id,rows:[],ok:false}; }
      }));
      if (!live) return;
      const batchesByIngredient = Object.fromEntries(batchResults.map(result=>[String(result.ingredient_id),result.rows]));
      const enrichedIngredients = ingredients.map(ingredient=>({...ingredient,_record_type:'ingredient',_batches:batchesByIngredient[String(ingredient.id)]||[]}));
      const menuItems = (byName.menu||[]).map(item=>({...item,_record_type:'menu'}));
      const batchEvidence = batchResults.flatMap(result=>result.rows.map(batch=>({...batch,ingredient_id:result.ingredient_id,_source:'ingredient_batches'})));
      const errors = results.filter(r=>!r.ok).map(r=>r.name);
      if (batchResults.some(result=>!result.ok)) errors.push('ingredient_batches');
      setSources({ inventory:[...enrichedIngredients,...menuItems], movements:batchEvidence, loading:false, errors });
    });
    return ()=>{ live=false; };
  },[api,branch,brand,scope]);

  const bounds = useMemo(() => {
    if (custom) return [new Date(`${custom.from}T00:00:00`),new Date(`${custom.to}T23:59:59.999`)];
    const end = new Date(); let start = new Date(end);
    if (period==='day') start.setHours(0,0,0,0);
    if (period==='week') { start.setDate(end.getDate()-6); start.setHours(0,0,0,0); }
    if (period==='month') start = new Date(end.getFullYear(),end.getMonth(),1);
    if (period==='year') start = new Date(end.getFullYear(),0,1);
    return [start,end];
  },[period,custom]);

  const completed = useMemo(() => (transactions||[]).filter(tx => {
    if (!scope(tx)) return false;
    const status = normText(firstValue(tx,['status','transaction_status','payment_status'],'completed'));
    const d = safeDate(firstValue(tx,['created_at','date','transaction_date','completed_at']));
    return (!status || ['completed','paid','success','successful'].includes(status)) && d && d>=bounds[0] && d<=bounds[1];
  }),[transactions,scope,bounds]);

  const txItems = useMemo(() => completed.flatMap((tx,ti) => {
    let items = firstValue(tx,['items','products','line_items','transaction_items'],[]);
    if (typeof items==='string') { try { items=JSON.parse(items); } catch { items=[]; } }
    return (Array.isArray(items)?items:[]).map((item,ii)=>({
      ...item,_tx:tx,_key:`${evidenceId(tx,ti)}-${ii}`,
      name:firstValue(item,['product_name','name','item_name','menu_name'],'Product not recorded'),
      category:firstValue(item,['category','product_category'],'Not recorded'),
      qty:Number(firstValue(item,['quantity','qty','units'],0))||0,
      price:numberOrNull(firstValue(item,['unit_price','price','selling_price'])),
      amount:numberOrNull(firstValue(item,['subtotal','amount','line_total']))
    }));
  }),[completed]);

  const revenue = completed.reduce((s,tx)=>s+(Number(firstValue(tx,['total','total_amount','grand_total','amount'],0))||0),0);
  const quantitySold = txItems.reduce((s,x)=>s+x.qty,0);
  const productRows = useMemo(() => {
    const map = new Map();
    txItems.forEach(x=>{ const key=normText(x.name); const r=map.get(key)||{name:x.name,category:x.category,qty:0,revenue:0,tx:new Set(),rows:[]}; r.qty+=x.qty; r.revenue+=x.amount ?? (x.price===null?0:x.price*x.qty); r.tx.add(evidenceId(x._tx)); r.rows.push(x); map.set(key,r); });
    return [...map.values()].map(r=>({...r,transactions:r.tx.size,avgPrice:r.qty?r.revenue/r.qty:null})).sort((a,b)=>b.revenue-a.revenue);
  },[txItems]);

  const movementInRange = useMemo(()=>sources.movements.filter(m=>{const d=safeDate(firstValue(m,['created_at','date','movement_date','received_at'])); return scope(m)&&d&&d>=bounds[0]&&d<=bounds[1];}),[sources.movements,scope,bounds]);
  const inventoryRows = useMemo(() => {
    const map=new Map();
    sources.inventory.filter(item=>item._record_type==='ingredient'&&scope(item)).forEach((item,i)=>{
      const inventoryKey=String(firstValue(item,['ingredient_id','stock_item_id','product_id','sku','id'],`item-${i}`));
      if(!map.has(inventoryKey)) map.set(inventoryKey,{rowKey:inventoryKey,item,movements:[]});
    });
    movementInRange.forEach((m,i)=>{
      const movementKey=String(firstValue(m,['ingredient_id','stock_item_id','product_id','sku','item_id'],`movement-${i}`));
      const entry=map.get(movementKey)||{rowKey:movementKey,item:{name:firstValue(m,['ingredient_name','product_name','item_name','sku'],'Unidentified SKU')},movements:[]}; entry.movements.push(m); map.set(movementKey,entry);
    });
    const kind=m=>normText(firstValue(m,['movement_type','type','action','_source']));
    const qty=m=>Math.abs(Number(firstValue(m,['quantity','qty','quantity_changed','amount'],0))||0);
    const sum=(ms,tests)=>ms.filter(m=>tests.some(t=>kind(m).includes(t))).reduce((s,m)=>s+qty(m),0);
    return [...map.values()].map(entry=>{
      const item=entry.item, ms=entry.movements;
      const opening=numberOrNull(firstValue(item,['opening_stock','opening_quantity','beginning_stock']));
      const received=sum(ms,['deliver','receipt','receive','stock in','stock_in']);
      const transfersIn=sum(ms,['transfer in','transfer_in']); const transfersOut=sum(ms,['transfer out','transfer_out']);
      const disposals=sum(ms,['disposal','waste','expired']); const adjustments=sum(ms,['adjust']);
      const recorded=numberOrNull(firstValue(item,['closing_stock','recorded_closing_stock','current_stock','stock','quantity']));
      const name=firstValue(item,['ingredient_name','product_name','name','item_name','sku'],'Unidentified SKU');
      const soldFromMovements=sum(ms,['sale','pos','sold']);
      const product=productRows.find(p=>normText(p.name)===normText(name));
      const recipeEvidence=[];
      let recipeSold=0;
      sources.inventory.filter(record=>record._record_type==='menu').forEach(menuItem=>{
        let recipe=firstValue(menuItem,['ingredients','recipe','linked_ingredients'],[]);
        if(typeof recipe==='string'){try{recipe=JSON.parse(recipe)}catch{recipe=[]}}
        if(!Array.isArray(recipe)) return;
        const menuSales=productRows.find(p=>normText(p.name)===normText(firstValue(menuItem,['product_name','name','item_name','menu_name'])));
        if(!menuSales) return;
        recipe.forEach(ing=>{
          const ingId=String(firstValue(ing,['ingredient_id','stock_item_id','product_id','id'],'unknown'));
          const ingName=firstValue(ing,['ingredient_name','stock_item_name','name','item_name']);
          if(ingId!==entry.rowKey && normText(ingName)!==normText(name)) return;
          const perProduct=Number(firstValue(ing,['qty_required','quantity_required','quantity','qty'],0))||0;
          const consumed=menuSales.qty*perProduct;
          recipeSold+=consumed;
          recipeEvidence.push({product:menuSales.name,productsSold:menuSales.qty,perProduct,consumed,unit:firstValue(ing,['unit','measurement_unit'],firstValue(item,['unit'],'unit'))});
        });
      });
      const sold=soldFromMovements || recipeSold || product?.qty || 0;
      const calculable=opening!==null && recorded!==null;
      const expected=calculable?opening+received+transfersIn-sold-disposals-transfersOut:null;
      const variance=expected===null?null:recorded-expected;
      const missingRefs=ms.filter(m=>!firstValue(m,['reference_no','reference_id','transaction_id','receipt_no','batch_no','id'])).length;
      const batchExpirations=(item._batches||[]).map(batch=>safeDate(batch.exp_date)).filter(Boolean).sort((a,b)=>a-b);
      const exp=batchExpirations[0]||safeDate(firstValue(item,['exp_date']));
      const daysToExpiry=exp?Math.ceil((exp-today)/86400000):null;
      let severity='Normal';
      if(variance!==null&&Math.abs(variance)>=10 || missingRefs>=2) severity='Critical';
      else if(variance!==null&&Math.abs(variance)>=5 || adjustments>0 || (daysToExpiry!==null&&daysToExpiry<=7)) severity='High';
      else if(variance!==null&&variance!==0 || missingRefs>0 || (daysToExpiry!==null&&daysToExpiry<=30)) severity='Medium';
      return {rowKey:entry.rowKey,name,item,movements:ms,recipeEvidence,opening,received,transfersIn,transfersOut,disposals,adjustments,sold,recorded,expected,variance,missingRefs,daysToExpiry,severity,impact:variance===null?0:Math.abs(variance)*(numberOrNull(firstValue(item,['cost_per_unit','cost']))||0)};
    });
  },[sources.inventory,movementInRange,scope,productRows,today]);

  const previous = useMemo(()=>{
    const duration=bounds[1]-bounds[0]+1, end=new Date(bounds[0].getTime()-1), start=new Date(end.getTime()-duration+1);
    const rows=(transactions||[]).filter(tx=>{const d=safeDate(firstValue(tx,['created_at','date','transaction_date','completed_at'])); return scope(tx)&&d&&d>=start&&d<=end;});
    return rows.reduce((s,tx)=>s+(Number(firstValue(tx,['total','total_amount','grand_total','amount'],0))||0),0);
  },[transactions,scope,bounds]);
  const change=previous>0?((revenue-previous)/previous)*100:null;

  const graph = useMemo(()=>{
    const map=new Map(); completed.forEach(tx=>{const d=safeDate(firstValue(tx,['created_at','date','transaction_date'])); if(!d)return; const key=period==='day'?`${String(d.getHours()).padStart(2,'0')}:00`:period==='year'?d.toLocaleDateString('en-PH',{month:'short'}):iso(d); const r=map.get(key)||{label:key,value:0,rows:[],order:period==='day'?d.getHours():period==='year'?d.getMonth():d.getTime()}; r.value+=Number(firstValue(tx,['total','total_amount','grand_total','amount'],0))||0;r.rows.push(tx);map.set(key,r);}); return [...map.values()].sort((a,b)=>a.order-b.order);
  },[completed,period]);
  const maxGraph=Math.max(1,...graph.map(x=>x.value));
  const points=graph.map((g,i)=>({x:55+(i/Math.max(graph.length-1,1))*720,y:16+130-(g.value/maxGraph)*130,...g}));

  const severityRank={Critical:4,High:3,Medium:2,Normal:1};
  const sortedInventory=useMemo(()=>[...inventoryRows].sort((a,b)=>{let av=a[sort.key],bv=b[sort.key]; if(sort.key==='severity'){av=severityRank[av];bv=severityRank[bv];} if(sort.key==='name'){av=normText(av);bv=normText(bv);return (sort.dir==='asc'?1:-1)*av.localeCompare(bv);} return (sort.dir==='asc'?1:-1)*((Number(av)||0)-(Number(bv)||0));}),[inventoryRows,sort]);
  const setSortKey=key=>setSort(s=>({key,dir:s.key===key&&s.dir==='desc'?'asc':'desc'}));
  const openTx=(title,rows)=>setDrill({title,type:'transactions',rows:[...rows].sort((a,b)=>(Number(firstValue(b,['total','amount'],0))||0)-(Number(firstValue(a,['total','amount'],0))||0))});
  const openStock=row=>setDrill({title:row.name,type:'stock',row});

  const runAi=async()=>{
    if(completed.length<3){setAi({loading:false,data:null,error:'Insufficient historical data'});return;}
    setAi({loading:true,data:null,error:null});
    try{const res=await fetch(`${api}/ai/dashboard-analysis`,{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({transactions:completed,preset:custom?`${custom.from} to ${custom.to}`:period,filterLabel:`${brand} — ${branch}`})}); const json=await res.json(); if(!res.ok)throw new Error(json.error||'AI service failed'); setAi({loading:false,data:json.analysis||json,error:null});}catch(e){setAi({loading:false,data:null,error:e.message});}
  };

  const noData = label => <span style={{ color: '#9a9aa2' }}>{label}</span>;
  const best=productRows[0];
  const peak=graph.length?[...graph].sort((a,b)=>b.value-a.value)[0]:null;
  const lowStock=inventoryRows.filter(r=>{const min=numberOrNull(firstValue(r.item,['min_stock','minimum_stock','reorder_level']));return min!==null&&r.recorded!==null&&r.recorded<=min;});
  const expiring=inventoryRows.filter(r=>r.daysToExpiry!==null&&r.daysToExpiry>=0&&r.daysToExpiry<=30);
  const alerts=inventoryRows.filter(r=>r.severity!=='Normal');
  const highPriority = alerts.filter(a=>['Critical','High'].includes(a.severity));
  ...
  const filteredProductRows = useMemo(() => {
    ...
    const rows = q ? productRows.filter(r => r.name.toLowerCase().includes(q)) : productRows;
    return [...rows].sort((a,b) => {
      const av = a[productSort.key], bv = b[productSort.key];
      if (typeof av === 'string') return productSort.dir==='asc' ? av.localeCompare(bv) : bv.localeCompare(av);
      return productSort.dir==='asc' ? (Number(av)||0)-(Number(bv)||0) : (Number(bv)||0)-(Number(av)||0);
    });
  }, [productRows, productSearch, productSort]);
  const setProductSortKey = key => setProductSort(s => ({ key, dir: s.key===key ? (s.dir==='asc'?'desc':'asc') : 'desc' }));

  const rangeLabel = custom ? `${custom.from} → ${custom.to}` : { day:'Today', week:'This week', month:'This month', year:'This year' }[period];
    ...
    <div className="bd-scope" style={{ fontFamily: FONT }}>
      ...
      <div className="bd-topbar">
        <div className="bd-brand">
          ...
          <div>
            <div className="bd-title">{branch || 'Branch not assigned'} <span className="bd-title-tag">{brand || 'Brand not assigned'}</span></div>
            <div className="bd-lock-note"><Lock size={11} /> Locked to your assigned branch · {completed.length} verified transaction{completed.length===1?'':'s'}</div>
          </div>
        </div>
      </div>
      ...
      <div className="bd-filterbar">
        {['day','week','month','year'].map(p => (
          <button key={p} className={`bd-pill ${!custom && period===p ? 'active' : ''}`} onClick={() => { setCustom(null); setPeriod(p); }}>
            {p[0].toUpperCase()+p.slice(1)}
          </button>
        ))}
        <Calendar size={13} color="#9a9aa2" />
        <input type="date" className="bd-date-input" value={from} onChange={e=>setFrom(e.target.value)} />
        <span style={{ fontSize:12, color:'#9a9aa2' }}>to</span>
        <input type="date" className="bd-date-input" value={to} onChange={e=>setTo(e.target.value)} />
        <button className="bd-apply-btn" onClick={()=>from&&to&&from<=to&&setCustom({from,to})}>Apply</button>
      </div>

      {tab === 'overview' && (
        <>
          <div className="bd-kpi-grid">
            <BdKpiCard icon={DollarSign} tone="green" label="Total sales revenue"
              value={completed.length ? Math.round(revenue) : 0}
              sub={completed.length ? `${completed.length} completed POS records` : 'No data available'}
              trend={change}
              onClick={() => openTx('Total Sales Revenue', completed)} />
            <BdKpiCard icon={Receipt} tone="neutral" label="Completed transactions"
              value={completed.length}
              sub="Completed / paid POS records only"
              onClick={() => openTx('Completed Transactions', completed)} />
            <BdKpiCard icon={ShoppingCart} tone="neutral" label="Average transaction value"
              value={completed.length ? Math.round(revenue / completed.length) : 0}
              sub="Revenue ÷ completed transactions"
              onClick={() => openTx('Average Transaction Value', completed)} />
            <BdKpiCard icon={TrendingUp} tone={hasCogs ? "green" : "neutral"} label="Gross profit"
              value={hasCogs ? Math.round(calculateTransactionProfit(completed)) : "—"}
              sub={hasCogs ? "Revenue − cost of sales" : "Cost data not recorded"}
              onClick={() => setDrill({ title: 'Gross Profit Calculation', type: 'profit', rows: completed })} />
          </div>

          <div className="bd-mini-grid">
            <BdMiniCard icon={BarChart2} label="Best-selling product" value={best?.name || 'No data available'}
              sub={best ? `${best.qty} units · ${fmtPeso(best.revenue)}` : 'POS item details unavailable'}
              onClick={best ? () => setDrill({ title: best.name, type: 'product', row: best }) : undefined} />
            <BdMiniCard icon={Clock} label="Busiest sales period" value={peak?.label || 'No data available'}
              sub={peak ? fmtPeso(peak.value) : 'No completed POS records'}
              onClick={peak ? () => openTx(`Sales for ${peak.label}`, peak.rows) : undefined} />
            <BdMiniCard icon={Package} label="Products sold" value={txItems.length ? quantitySold : 'No data available'}
              sub={`${productRows.length} distinct product(s)`}
              onClick={() => setDrill({ title: 'Products Sold', type: 'products', rows: productRows })} />
          </div>
          ...
          <div className="bd-layout">
            <div className="bd-card">
              <div className="bd-card-head">
                <div>
                  <div className="bd-card-title"><Package size={14} /> Branch performance evidence</div>
                  <div className="bd-card-sub">Highest revenue first · click a row for supporting POS records</div>
                </div>
                <div className="bd-search"><Search size={13} color="#9a9aa2" /><input placeholder="Search products…" value={productSearch} onChange={e=>setProductSearch(e.target.value)} /></div>
              </div>
              <div className="bd-card-body" style={{ padding: 8 }}>
                <ProductEvidenceTable rows={filteredProductRows} sort={productSort} onSort={setProductSortKey} onOpen={row=>setDrill({title:row.name,type:'product',row})} />
              </div>
            </div>

            <div className="bd-card">
              <div className="bd-card-head"><div className="bd-card-title"><AlertTriangle size={14} /> Needs attention</div></div>
              <div className="bd-card-body" style={{ display:'flex', flexDirection:'column', gap:2 }}>
                {[
                  { label:'Low-stock items', sub: sources.errors.includes('inventory') ? 'Incomplete stock movement records' : 'At or below reorder level', count: sources.loading ? '…' : lowStock.length, sev:'High', rows: lowStock },
                  { label:'Expiring items', sub:'Within 30 days of recorded expiry', count: sources.loading ? '…' : expiring.length, sev:'Medium', rows: expiring },
                  { label:'High-priority alerts', sub:'Critical and High inventory findings', count: sources.loading ? '…' : highPriority.length, sev:'Critical', rows: highPriority },
                ].map((a,i) => {
                  const s = severityStyle(a.sev);
                  return (
                    <div key={i} className="bd-attn-row" onClick={() => setDrill({ title: a.label, type:'inventory', rows: a.rows })}>
                ...
                <div style={{ marginTop:8, padding:'10px 12px', borderRadius:10, background:'var(--surface-soft)', border:'1px dashed var(--border)', fontSize:11.5, color:'var(--text-mute)', lineHeight:1.5 }}>
      ...
      {tab === 'sales' && (
        <>
          <div className="bd-mini-grid">
            <BdMiniCard icon={DollarSign} label="Current-period revenue" value={completed.length ? fmtPeso(revenue) : 'No data available'} sub="Verified completed POS only" onClick={() => openTx('Current-period Revenue', completed)} />
            <BdMiniCard icon={Clock} label="Previous comparable period" value={previous ? fmtPeso(previous) : 'No data available'} sub="Same duration immediately before" />
            <BdMiniCard icon={change>=0 ? TrendingUp : TrendingDown} label="Percentage change" value={change===null ? 'No comparable data' : `${change>=0?'+':''}${change.toFixed(1)}%`} sub="(Current − previous) ÷ previous" />
          </div>

          <div className="bd-card" style={{ marginBottom:14 }}>
            <div className="bd-card-head">
              <div>
                <div className="bd-card-title"><BarChart2 size={14} /> Sales trend analysis</div>
                <div className="bd-card-sub">{rangeLabel} · click a point to inspect that period's transactions</div>
              </div>
            </div>
            <div className="bd-card-body">
              {points.length
                ? <BdTrendChart points={points} maxGraph={maxGraph} onPointClick={p => openTx(`Sales for ${p.label}`, p.rows)} />
                : <VEmptyState icon={LineChart} title="No data available" sub="No completed POS transactions exist for the selected period." />}
            </div>
          </div>
          ...
          <div className="bd-card">
            <div className="bd-card-head">
              <div>
                <div className="bd-card-title"><Brain size={14} /> AI prescriptive analysis <span className="bd-ai-tag" style={{ marginLeft:6 }}>AI-generated</span></div>
                <div className="bd-card-sub">Generated only from the verified branch evidence above</div>
              </div>
              <button className="bd-ai-btn" onClick={runAi} disabled={ai.loading}>
                {ai.loading ? <RefreshCw size={13} style={{ animation:'bd-spin .7s linear infinite' }} /> : <Sparkles size={13} />}
                {ai.loading ? 'Analyzing…' : ai.data ? 'Re-run analysis' : 'Run AI analysis'}
              </button>
            </div>
            <div className="bd-card-body">
              {!ai.loading && !ai.data && !ai.error && (
                <div style={{ textAlign:'center', padding:'26px 10px', border:'1px dashed var(--border)', borderRadius:12, color:'var(--text-mute)', fontSize:12.5 }}>
                  Run the analysis to get evidence-based recommendations for this branch.
                </div>
              )}
              {ai.loading && (
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  {[0,1,2].map(i => <div key={i} className="bd-skel" style={{ height:58 }} />)}
                </div>
              )}
              ...
              {ai.data && (
                <AIRecommendations
                  ...
                  from={bounds[0]}
                  to={bounds[1]}
                  onEvidence={(name) => { const p = productRows.find(x => normText(x.name)===normText(name)); if (p) setDrill({ title:p.name, type:'product', row:p }); }}
                />
              )}
            </div>
          </div>
      ...
      {tab === 'ghost' && (
        <div className="bd-card">
          <div className="bd-card-head">
            <div>
              <div className="bd-card-title"><Layers size={14} /> Stock evidence</div>
              <div className="bd-card-sub">Expected = opening + received + transfers in − sold − disposals − transfers out</div>
            </div>
          </div>
          <div className="bd-card-body">
            {sources.loading ? (
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                {[0,1,2,3].map(i => <div key={i} className="bd-skel" style={{ height:44 }} />)}
              </div>
            ) : (
              <>
                <div style={{ display:'flex', gap:7, flexWrap:'wrap', marginBottom:12 }}>
                  {[['severity','Severity'],['variance','Largest variance'],['impact','Financial impact'],['daysToExpiry','Earliest expiry'],['name','Product / SKU']].map(([key,label]) => (
                    <button key={key} className={`bd-pill ${sort.key===key ? 'active' : ''}`} onClick={() => setSortKey(key)}>
                ...
                {sources.errors.length > 0 && (
                  <div className="bd-banner-warn">
                    Incomplete stock movement records: unavailable source(s): {sources.errors.join(', ')}. Variance is shown as "Unable to calculate" where required fields are missing.
                  </div>
                )}
                <InventoryEvidenceTable rows={sortedInventory} onOpen={openStock} />
              </>
            )}
          </div>
        </div>
      ...
      {drill && <EvidenceModal drill={drill} brand={brand} branch={branch} onClose={() => setDrill(null)} />}
    </div>
  );
}
...
function ProductEvidenceTable({ rows, sort, onSort, onOpen }) {
  if (!rows.length) return <VEmptyState icon={Package} title="No data available" sub="Product-level POS line items were not recorded for this period." />;
  const cols = [
    ['name','Product'], ['category','Category'], ['qty','Quantity sold'],
    ['transactions','Transactions'], ['revenue','Sales revenue'], ['avgPrice','Average price'],
  ];
  return (
    <table className="bd-table">
      <thead>
        <tr>
          {cols.map(([key,label]) => (
            <th key={key} onClick={() => onSort && onSort(key)} style={{ textAlign: ['qty','transactions','revenue','avgPrice'].includes(key) ? 'right' : 'left' }}>
      ...
      <tbody>
        {rows.map(r => (
          <tr key={r.name} className="bd-row" onClick={() => onOpen(r)}>
            ...
            <td style={{ textAlign:'right', fontWeight:700, color:'var(--text)' }}>{fmtPeso(r.revenue)}</td>
            <td style={{ textAlign:'right' }}>{r.avgPrice===null ? 'Not recorded' : fmtPeso(r.avgPrice)}</td>
            <td style={{ width:20 }}><ChevronRight size={14} className="bd-row-chevron" /></td>
          </tr>
        ))}
      </tbody>
    </table>
...
function InventoryEvidenceTable({ rows, onOpen }) {
  if (!rows.length) return <VEmptyState icon={FileCheck} title="No data available" sub="No branch-scoped inventory or stock movement records were returned." />;
  return (
    <table className="bd-table">
      <thead>
        <tr><th>Severity</th><th>Ingredient / SKU</th><th style={{textAlign:'right'}}>Expected</th><th style={{textAlign:'right'}}>Recorded</th><th style={{textAlign:'right'}}>Variance</th><th>Missing refs</th><th>Expiry</th><th /></tr>
      </thead>
      <tbody>
        {rows.map(r => {
          ...
          return (
            <tr key={r.rowKey} className="bd-row" onClick={() => onOpen(r)}>
              <td><span className="bd-chip" style={{ background:s.bg, color:s.text, border:`1px solid ${s.border}` }}><span className="bd-dot" style={{ background:s.dot }} />{r.severity}</span></td>
              <td className="bd-name-cell">{r.name}</td>
              <td style={{ textAlign:'right' }}>{r.expected===null ? 'Unable to calculate' : r.expected}</td>
              <td style={{ textAlign:'right' }}>{r.recorded===null ? 'Not recorded' : r.recorded}</td>
              <td style={{ textAlign:'right', fontWeight:800, color: r.variance===null ? 'var(--text-mute)' : r.variance===0 ? 'var(--green-dark)' : 'var(--red)' }}>
                {r.variance===null ? 'Unable to calculate' : `${r.variance>0?'+':''}${r.variance}`}
              </td>
              <td>{r.missingRefs}</td>
              <td>{r.daysToExpiry===null ? 'Not recorded' : r.daysToExpiry<0 ? 'Expired' : `${r.daysToExpiry} day(s)`}</td>
              <td style={{ width:20 }}><ChevronRight size={14} className="bd-row-chevron" /></td>
            </tr>
          );
        })}
...
function AIRecommendations({ data, from, to, onEvidence }) {
  ...
  if (!list.length) return <VEmptyState icon={Brain} title="No recommendation returned" sub="The AI service did not return structured recommendations." />;
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:9 }}>
      {list.map((r,i) => {
        ...
        return (
          <div key={i} className="bd-ai-card" style={{ background:s.bg, borderColor:s.border, animationDelay:`${i*80}ms` }}>
            <div style={{ display:'flex', alignItems:'center', gap:7, marginBottom:6, flexWrap:'wrap' }}>
              <span className="bd-dot" style={{ background:s.dot }} />
              <strong style={{ fontSize:12.8 }}>{r.product || r.ingredient || 'Branch-level recommendation'}</strong>
              <span className="bd-ai-tag">AI-generated</span>
              <span className="bd-chip" style={{ background:s.bg, color:s.text, border:`1px solid ${s.border}` }}>{r.priority || 'Priority not supplied'}</span>
            </div>
            <div style={{ fontSize:12, color:'var(--text-soft)', lineHeight:1.65 }}>
              <b>Issue:</b> {r.issue || 'Not supplied'}<br/>
              <b>Supporting evidence:</b> {r.evidence || r.supportingEvidence || 'Not supplied'}<br/>
              <b>Business impact:</b> {r.impact || r.businessImpact || 'Not supplied'}<br/>
              <b>Recommended action:</b> {r.action || r.recommendedAction || r.text || 'Not supplied'}<br/>
              <b>Confidence:</b> {r.confidence ?? 'Not supplied'} · <b>Date range:</b> {from.toLocaleDateString('en-PH')}–{to.toLocaleDateString('en-PH')}
            </div>
            {(r.product || r.ingredient) && (
              <button className="bd-pill" style={{ marginTop:9, height:28 }} onClick={() => onEvidence(r.product || r.ingredient)}>Open supporting records</button>
            )}
          </div>
        );
      })}
...
function EvidenceModal({ drill, brand, branch, onClose }) {
  const txTable = rows => (
    <table className="bd-table">
      <thead><tr><th>Source / ID</th><th>Date &amp; time</th><th>User</th><th style={{textAlign:'right'}}>Quantity</th><th style={{textAlign:'right'}}>Unit price / cost</th><th style={{textAlign:'right'}}>Amount</th><th>Completeness</th></tr></thead>
      <tbody>
        {rows.map((r,i) => {
          const tx = r._tx || r;
          const date = firstValue(tx, ['created_at','supply_date','transaction_date']);
          ...
          return (
            <tr key={r._key || evidenceId(tx,i)}>
              <td><b>{r._tx ? 'POS line item' : r._source==='ingredient_batches' ? 'Ingredient batch' : 'POS'}</b><br/>{evidenceId(tx,i)}</td>
              <td>{safeDate(date)?.toLocaleString('en-PH') || 'Not recorded'}</td>
              <td>{actor || 'Not recorded'}</td>
              <td style={{ textAlign:'right' }}>{firstValue(r, ['qty','quantity','stock'], 'Not recorded')}</td>
              <td style={{ textAlign:'right' }}>{numberOrNull(firstValue(r, ['price','cost_per_unit']))===null ? 'Not recorded' : fmtPeso(firstValue(r, ['price','cost_per_unit']))}</td>
              <td style={{ textAlign:'right', fontWeight:700, color:'var(--text)' }}>{r._tx ? fmtPeso(r.amount ?? ((r.price||0)*r.qty)) : firstValue(r, ['total'], 'Not applicable')}</td>
              <td>
                <span className="bd-chip" style={complete
                  ? { background:'var(--green-light)', color:'var(--green-dark)', border:'1px solid var(--green-line)' }
                  : { background:'var(--amber-bg)', color:'var(--amber)', border:'1px solid #f3dfb8' }}>
  ...
  let body = null;
  if (drill.type === 'transactions') body = drill.rows.length ? txTable(drill.rows) : <VEmptyState icon={Receipt} title="No data available" sub="No completed POS source records are included." />;
  if (drill.type === 'product') body = (
    <>
      <div style={{ marginBottom:14, fontSize:12.5, color:'var(--text-soft)' }}>
        <b>Category:</b> {drill.row.category} · <b>Quantity:</b> {drill.row.qty} · <b>Revenue:</b> {fmtPeso(drill.row.revenue)} · <b>Calculation:</b> sum of POS line amounts for this product
      </div>
      {txTable(drill.row.rows)}
    </>
  );
  if (drill.type === 'products') body = <ProductEvidenceTable rows={drill.rows} onOpen={() => {}} />;
  if (drill.type === 'profit') body = (
    <>
      <div style={{ marginBottom:14, fontSize:12.5, color:'var(--text-soft)' }}>
        <b>Calculation:</b> Σ transaction total − Σ transaction cogs = {fmtPeso(calculateTransactionProfit(drill.rows))}
      </div>
      {txTable(drill.rows)}
    </>
  );
  if (drill.type === 'inventory') body = <InventoryEvidenceTable rows={drill.rows} onOpen={() => {}} />;
  if (drill.type === 'stock') {
    const r = drill.row;
    body = (
      <>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:8, marginBottom:14 }}>
          {[['Opening',r.opening],['Received',r.received],['Transfers in',r.transfersIn],['POS / recipe consumed',r.sold],['Disposals',r.disposals],['Transfers out',r.transfersOut],['Manual adjustments',r.adjustments],['Expected',r.expected],['Recorded',r.recorded],['Variance',r.variance]].map(([k,v]) => (
            <div key={k} style={{ padding:10, background:'var(--surface-soft)', border:'1px solid var(--border-soft)', borderRadius:10 }}>
              <div style={{ fontSize:10.5, color:'var(--text-mute)', fontWeight:700, textTransform:'uppercase', letterSpacing:'.04em' }}>{k}</div>
              <div style={{ fontWeight:800, fontSize:14, marginTop:2 }}>{v===null ? 'Not recorded' : v}</div>
            </div>
          ))}
        </div>
        <div style={{ fontSize:11.5, color:'var(--text-mute)', marginBottom:14, lineHeight:1.6 }}>
          <b style={{ color:'var(--text-soft)' }}>Calculation:</b> {r.opening ?? '?'} + {r.received} + {r.transfersIn} − {r.sold} − {r.disposals} − {r.transfersOut} = {r.expected ?? 'Unable to calculate'}<br/>
          <b style={{ color:'var(--text-soft)' }}>SKU:</b> {r.item.sku || 'Not recorded'} · <b style={{ color:'var(--text-soft)' }}>Cost per unit:</b> {r.item.cost_per_unit == null ? 'Not recorded' : fmtPeso(r.item.cost_per_unit)} · <b style={{ color:'var(--text-soft)' }}>Missing references:</b> {r.missingRefs}
        </div>
        {r.recipeEvidence?.length > 0 && (
          <div style={{ marginBottom:16 }}>
            <div style={{ fontSize:12.5, fontWeight:800, marginBottom:8 }}>Menu ingredient consumption evidence</div>
            <table className="bd-table">
              <thead><tr><th>Sold product</th><th style={{textAlign:'right'}}>Products sold</th><th style={{textAlign:'right'}}>Required per product</th><th style={{textAlign:'right'}}>Total ingredient consumed</th></tr></thead>
              <tbody>
                {r.recipeEvidence.map((x,i) => (
                  <tr key={`${x.product}-${i}`}>
                    <td>{x.product}</td>
                    <td style={{ textAlign:'right' }}>{x.productsSold}</td>
                    <td style={{ textAlign:'right' }}>{x.perProduct} {x.unit}</td>
                    <td style={{ textAlign:'right', fontWeight:700 }}>{x.consumed} {x.unit}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {r.movements.length ? txTable(r.movements) : <VEmptyState icon={FileText} title="Incomplete stock movement records" sub="No ingredient batch evidence was returned for this ingredient or SKU." />}
      </>
    );
  }
  ...
  return (
    <div
      ...
      style={{
        ...
        background:'rgba(36,49,12,0.46)', backdropFilter:'blur(5px)', zIndex:99999,
      }}
    >
      <div
        ...
        style={{
          ...
          width:'min(880px, calc(100vw - 48px))', maxHeight:'82vh', margin:'0 auto',
          background:C.white, border:`1px solid ${C.greenMid}`, borderRadius:20,
          boxShadow:'0 26px 75px rgba(36,49,12,0.32)', overflow:'hidden',
      ...
      >
        <div style={{ padding:'16px 20px', background:C.bg, borderBottom:`1px solid ${C.border}`, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div>
            <div style={{ fontWeight:800, fontSize:14.5, color:C.greenDk }}>{drill.title}</div>
            <div style={{ fontSize:11.5, color:C.muted, marginTop:2 }}>{brand} → {branch} → evidence source records</div>
          </div>
          <button className="bd-icon-btn" onClick={onClose} style={{borderColor:C.border,color:C.greenDk,background:C.white}}><X size={14} /></button>
        </div>
        <div style={{ padding:'14px 16px 20px', overflowY:'auto', background:C.white }}>{body}</div>
      </div>
    </div>
  );
}
...
function MaDashboardContent({ transactions, brands, user }) {
  ...
  const userBrand = String(user?.brand || user?.brand_name || user?.brandName || brands?.[0]?.name || '').trim();
  const today      = new Date();
  const fmt8       = (d) => d.toISOString().slice(0, 10);
...
const [archives, setArchives] = useState(() => {
  ...
  return (transactions || []).filter(tx =>
    ...
    (!userBrand || !String(tx.brand || tx.brand_name || '').trim() || String(tx.brand || tx.brand_name || '').trim().toLowerCase() === userBrand.toLowerCase())
  );
}, [transactions, userBranch, userBrand]);


const tabSt = (a) => ({ padding: "6px 13px", borderRadius: 9, border: "none", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: FONT, transition: "all .15s", background: a ? "linear-gradient(135deg,#509820,#3b791e)" : "transparent", color: a ? "#fff" : "#5C6B60", boxShadow: a ? "0 2px 8px rgba(59,121,30,.35)" : "none" });
  ...
  const periodTransactions = useMemo(() => {
    ...
    return myTransactions.filter(tx => {
      ...
      if (Number.isNaN(d.getTime())) return false;
      if (rangeMode === 'custom' && appliedRange) {
        return d >= new Date(`${appliedRange.from}T00:00:00`) && d <= new Date(`${appliedRange.to}T23:59:59.999`);
      }
      if (preset === 'day') return d.toDateString() === now.toDateString();
      if (preset === 'week') {
        const start = new Date(now); start.setDate(now.getDate() - now.getDay()); start.setHours(0,0,0,0);
...
const low       = useMemo(() => values.length ? Math.min(...values) : 0, [values]);
  ...
  const sortedBranchTransactions = useMemo(() => [...periodTransactions].sort((a, b) =>
  ...
  return (
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <div className="manager-dashboard-tabs">
        {[
          { id:'overview', number:'01', label:'Overview', question:'How is my branch performing?', icon:Home },
          { id:'sales_ai', number:'02', label:'Sales & AI Analysis', question:'Why are sales changing?', icon:LineChart },
          { id:'stock_products', number:'03', label:'Stock & Product Performance', question:'What should I reorder or improve?', icon:Layers },
        ].map(tab => {
          ...
          return (
            <button
              ...
              style={{
                ...
                background:active ? 'linear-gradient(135deg,#F2F7EB,#EAF3DF)' : 'transparent',
                color:active ? '#2c5c16' : '#64748b',
                boxShadow:active ? 'inset 0 0 0 1px rgba(59,121,30,.05)' : 'none',
              }}
            >
              <span style={{ width:34, height:34, borderRadius:10, flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', background:active ? '#3b791e' : '#F1F5F0', color:active ? '#bdd43c' : '#71806F' }}><Icon size={16} /></span>
              <span style={{ minWidth:0 }}>
                <span style={{ display:'block', fontSize:9, fontWeight:900, letterSpacing:'.08em', opacity:.72, marginBottom:2 }}>{tab.number}</span>
                <span style={{ display:'block', fontSize:11.4, fontWeight:850, lineHeight:1.25 }}>{tab.label}</span>
                <span style={{ display:'block', fontSize:9.4, color:active ? '#5C6B60' : '#9CA89C', fontWeight:650, marginTop:3, lineHeight:1.25 }}>{tab.question}</span>
              </span>
            </button>
          );
        })}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, margin:'18px 0 12px', color:'#5C6B60' }}>
        <span style={{ height:1, background:'#DCE9DB', flex:1 }} />
        <span style={{ fontSize:10, fontWeight:850, letterSpacing:'.11em', textTransform:'uppercase', whiteSpace:'nowrap' }}>
          {dashboardTab === 'overview' ? 'Branch Performance Workspace' : dashboardTab === 'sales_ai' ? 'Sales & AI Decision Workspace' : 'Stock & Product Decision Workspace'}
        </span>
        <span style={{ height:1, background:'#DCE9DB', flex:1 }} />
      </div>
      ...
      {/* ── Filter + Date toolbar ── */}
      <div style={{ background: "#fff", border: "1px solid rgba(59,121,30,0.12)", borderRadius: 14, padding: "12px 16px", marginBottom: 14, boxShadow: "0 1px 8px rgba(50,109,32,0.05)", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        ...
        {/* Archive */}
        <button onClick={() => setShowArchivePanel(v => !v)} style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", borderRadius: 9, border: "1.5px solid #D4DBC8", background: showArchivePanel ? "#E1E6D8" : "#fff", color: "#2c5c16", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: FONT }}>
      ...
      <div className="fr-db-kpi-grid">
        {[
          { label: "Today's Revenue", value:fmtPeso(todayRevenue), sub:`${todaySales.length} completed transactions`, icon:<DollarSign size={17}/>, color:'#3b791e', bg:'#F2F7EB', rows:todaySales },
          { label:'Period Revenue', value:kpiLoading?'…':fmtPeso(kpiData?.salesRevenue ?? 0), sub:`${getRangeLabel()} · ${userBranch || 'Branch'}`, icon:<BarChart size={17}/>, color:'#3b791e', bg:'#F2F7EB', rows:sortedBranchTransactions },
          { label:'Period Profit', value:kpiLoading?'…':(kpiData?.salesProfit == null ? 'Not available' : fmtPeso(kpiData.salesProfit)), sub:'Requires recorded cost of sales', icon:<TrendingUp size={17}/>, color:'#3b791e', bg:'#F2F7EB', rows:sortedBranchTransactions },
          { label:'Average Sale', value:fmtPeso(isNaN(avgOrder)?0:avgOrder), sub:'Revenue per transaction today', icon:<ShoppingCart size={17}/>, color:'#3b791e', bg:'#F2F7EB', rows:todaySales },
        ].map((k,i)=>(
          <div key={i} className="fr-db-kpi" role="button" tabIndex={0}
            onClick={() => setDashboardDrilldown({ title:k.label, rows:[...k.rows].sort((a,b)=>Number(b.total||0)-Number(a.total||0)) })}
            onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') setDashboardDrilldown({ title:k.label, rows:[...k.rows].sort((a,b)=>Number(b.total||0)-Number(a.total||0)) }); }}
            style={{cursor:'pointer'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
              <div style={{fontSize:10,fontWeight:850,textTransform:'uppercase',letterSpacing:'.08em',color:'#3b791e'}}>{k.label}</div>
              <div style={{width:31,height:31,borderRadius:9,background:k.bg,color:k.color,display:'flex',alignItems:'center',justifyContent:'center'}}>{k.icon}</div>
            </div>
            <div style={{fontSize:23,fontWeight:850,color:'#12241B',letterSpacing:'-.02em'}}>{k.value}</div>
            <div style={{fontSize:10.5,fontWeight:600,color:'#9CA89C',marginTop:6,lineHeight:1.45}}>{k.sub}</div>
            <div style={{fontSize:9.5,fontWeight:800,color:'#3b791e',marginTop:8}}>View sorted breakdown →</div>
          </div>
        ))}
      </div>

      <div style={{background:'#fff',border:'1px solid #DCE9DB',borderLeft:'4px solid #3b791e',borderRadius:14,padding:'14px 17px',marginBottom:18,boxShadow:'0 2px 10px rgba(50,109,32,.04)'}}>
        <div style={{display:'flex',alignItems:'center',gap:8,fontSize:13,fontWeight:850,color:'#12241B',marginBottom:4}}>
          {dashboardTab === 'overview' ? <Activity size={15} color="#3b791e"/> : dashboardTab === 'sales_ai' ? <LineChart size={15} color="#3b791e"/> : <Layers size={15} color="#3b791e"/>}
          {dashboardTab === 'overview' ? 'Start with the branch performance summary' : dashboardTab === 'sales_ai' ? 'Read the actual sales evidence before the AI guidance' : 'Compare stock movement with actual product sales'}
        </div>
      ...
      {/* Archive panel */}
      {showArchivePanel && (
        <div style={{ background: "#fff", border: "1px solid rgba(59,121,30,0.15)", borderRadius: 16, padding: "18px 20px", boxShadow: "0 2px 16px rgba(50,109,32,0.08)", marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              {!archiveConfirm ? (
                <>
                  ...
                  <button onClick={() => setArchiveConfirm(true)} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", borderRadius: 8, border: "none", background: "linear-gradient(135deg,#3b791e,#3b791e)", color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: FONT }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#fef9c3", border: "1.5px solid #fde68a", borderRadius: 9, padding: "6px 12px" }}>
                  ...
                  <button onClick={saveArchive} style={{ padding: "4px 11px", borderRadius: 7, fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: FONT, border: "1px solid #3b791e", background: "#E1E6D8", color: "#2c5c16" }}>Confirm</button>
          ...
          {archives.length === 0
            ? <div style={{ padding: "20px 0", textAlign: "center", color: "#9CA89C", fontSize: 13, fontFamily: FONT }}>No archives yet.</div>
            : archives.map(a => (
              <div key={a.year} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 13px", borderRadius: 9, border: "1px solid #E1E6D8", marginBottom: 7, background: "#fbfdf6" }}>
                <div>
                ...
                <div style={{ display: "flex", gap: 7 }}>
                  <button onClick={() => { setViewingArchive(viewingArchive?.year === a.year ? null : a); setShowArchivePanel(false); }} style={{ padding: "4px 11px", borderRadius: 7, fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: FONT, border: `1px solid ${viewingArchive?.year === a.year ? "#3b791e" : "#D4DBC8"}`, background: viewingArchive?.year === a.year ? "#E1E6D8" : "#fbfdf6", color: "#2c5c16" }}>
      ...
      {dashboardTab === 'overview' && <BranchOperationsSnapshot
      ...
      {dashboardDrilldown && (
        <div className="v-modal-overlay" onClick={() => setDashboardDrilldown(null)}>
          <div className="v-modal" style={{maxWidth:760,padding:0}} onClick={e=>e.stopPropagation()}>
            <div style={{padding:'18px 20px',borderBottom:'1px solid #E1E6D8',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <div><div style={{fontSize:16,fontWeight:850,color:'#12241B'}}>{dashboardDrilldown.title}</div><div style={{fontSize:10.5,color:'#5C6B60',marginTop:3}}>{userBranch} · highest-value transactions first</div></div>
              <button onClick={()=>setDashboardDrilldown(null)} style={{width:30,height:30,borderRadius:9,border:'1px solid #E1E6D8',background:'#fff',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}><X size={15}/></button>
            </div>
            <div style={{padding:20,maxHeight:'65vh',overflowY:'auto'}}>
              {dashboardDrilldown.rows.length === 0 ? <VEmptyState icon={BarChart2} title="No branch data" sub="No completed transactions are available for this selection."/> :
                <table className="v-table"><thead><tr><th>Transaction</th><th>Date</th><th>Payment</th><th style={{textAlign:'right'}}>Amount</th></tr></thead><tbody>
                  {dashboardDrilldown.rows.map((tx,index)=><tr key={tx.id || tx.transaction_id || index}><td style={{fontWeight:750}}>{tx.transaction_id || tx.reference_no || tx.id || `Transaction ${index+1}`}</td><td>{new Date(tx.created_at || tx.date).toLocaleString('en-PH')}</td><td>{tx.payment_method || tx.paymentMethod || tx.payment_type || 'Not recorded'}</td><td style={{textAlign:'right',fontWeight:800,color:'#3b791e'}}>{fmtPeso(tx.total)}</td></tr>)}
                </tbody></table>}
            </div>
          </div>
        </div>
...
${topItems}
      ...
      const data = await res.json();
      const reportText = data.content?.[0]?.text || 'Failed to generate report.No data available for the selected period.'; 
...
const submitReport = async report => {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '11px 16px', borderTop: '1px solid rgba(59,121,30,0.1)', background: '#f9fefb' }}>
      ...
      <div style={{ display: 'flex', gap: 4 }}>
        <button aria-label="First page" title="First page" onClick={() => setPage(0)} disabled={page === 0} className="v-btn v-btn-secondary v-btn-sm" style={{ opacity: page === 0 ? 0.35 : 1 }}><ArrowLeft size={12}/><ArrowLeft size={12} style={{ marginLeft:-9 }}/></button>
        <button aria-label="Previous page" title="Previous page" onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0} className="v-btn v-btn-secondary v-btn-sm" style={{ opacity: page === 0 ? 0.35 : 1 }}><ArrowLeft size={12}/></button>
        <button aria-label="Next page" title="Next page" onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1} className="v-btn v-btn-secondary v-btn-sm" style={{ opacity: page >= totalPages - 1 ? 0.35 : 1 }}><ArrowRight size={12}/></button>
        <button aria-label="Last page" title="Last page" onClick={() => setPage(totalPages - 1)} disabled={page >= totalPages - 1} className="v-btn v-btn-secondary v-btn-sm" style={{ opacity: page >= totalPages - 1 ? 0.35 : 1 }}><ArrowRight size={12}/><ArrowRight size={12} style={{ marginLeft:-9 }}/></button>
      </div>
    </div>
  );
};
  ...
  return (
    <div className="ma-reports" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      <style>{`
        .ma-reports {
      ...
      <div className="ma-page-head">
        <div>
          <h2 className="ma-page-title">
            <span className="ma-page-icon"><FileText size={19}/></span>
            Sales &amp; Reports
          </h2>
          <p className="ma-page-sub">Generate, review, and submit evidence-based branch sales reports.</p>
        </div>
        <div className="ma-context"><span className="ma-context-dot"/>{user?.brand || 'Assigned brand'} · {branch || 'Assigned branch'}</div>
      </div>
      ...
      <div className="v-card" style={{ padding:'20px 22px', marginBottom:16 }}>
        ...
        <div className="ma-generate-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 14, alignItems: 'end', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 11, fontWeight: 700, color: '#9CA89C', alignSelf: 'center', fontFamily: 'Plus Jakarta Sans,sans-serif', textTransform: 'uppercase', letterSpacing: '.06em' }}>Quick:</span>
          {[
            ...
            <button key={p.label} aria-pressed={dateFrom === p.from && dateTo === p.to} className={`v-btn v-btn-ghost v-btn-sm ${dateFrom === p.from && dateTo === p.to ? 'ma-preset-active' : ''}`} onClick={() => { setDateFrom(p.from); setDateTo(p.to); }}>{p.label}</button>
          ))}
        </div>
      ...
      <div className="ma-report-tabs">
        {[
          ...
          return (
            <button key={t.id} aria-pressed={active} onClick={()=>setReportTab(t.id)} className={`ma-report-tab ${active ? 'active' : ''}`}>
              <Icon size={13}/>{t.label}<span className="ma-report-count">{t.count}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className="v-card" style={{ padding:'18px 20px', marginBottom:16 }}>
        <div className="v-section-head">
          ...
          <span style={{ fontSize: 12, color: '#9CA89C', fontFamily: 'Plus Jakarta Sans,sans-serif' }}>{reports.length} pending submission</span>
        </div>
        {reports.length === 0 ? (
...
{/* Submitted Reports */}
    ...
    <div className="v-card" style={{ padding:'18px 20px', marginBottom:16 }}>
      <div className="v-section-head">
        ...
        <span style={{ fontSize: 12, color: '#9CA89C', fontFamily: 'Plus Jakarta Sans,sans-serif' }}>
              ...
              {submittedReports.slice(subPage * PAGE_SIZE, (subPage + 1) * PAGE_SIZE).map(h => (
                <React.Fragment key={h.id}>
                  <tr>
                    ...
                    <td style={{ fontSize: 12, color: '#9CA89C', fontFamily: 'Plus Jakarta Sans,sans-serif' }}>
                      {h.generatedDate || '—'}
                    </td>
<td>
  {(() => {
                      ...
                      <td colSpan={6} style={{ padding: 0, border: 'none' }}>
                        <div style={{ margin: '8px 0 12px', background: 'linear-gradient(135deg,rgba(59,121,30,0.04),rgba(59,121,30,0.03))', border: '1.5px solid rgba(59,121,30,0.15)', borderRadius: 14, padding: '18px 20px' }}>
                          ...
                          {h.content ? (
                            ...
                            <div style={{ padding: '24px 0', textAlign: 'center', color: '#9CA89C', fontSize: 13, fontStyle: 'italic' }}>
...
function FrCommunicationContent() {
  ...
  const commStyles = {
    ...
    labelAccent: {
      ...
      background: "linear-gradient(135deg,#3b791e,#4CAF50)", flexShrink: 0,
    },
    labelTxt: {
    ...
    recentBadge: {
      background: "#E1E6D8", borderRadius: 6, padding: "2px 6px",
      border: "1px solid #D4DBC8", fontSize: 8, fontWeight: 800, color: "#2c5c16",
    },
    ...
    emptyIcon: { display:'flex', justifyContent:'center', color:'#3b791e', marginBottom: 4 },
      ...
      <div style={commStyles.header}>
        ...
        {searchVisible && (
          <div style={commStyles.searchBarWrap}>
            ...
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery("")} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.7)", lineHeight: 1, display:'flex', alignItems:'center' }}><X size={15} /></button>
            )}
          </div>
        )}
      </div>
        ...
        {["all", "recent", "pinned"].map(tab => {
          ...
          return (
            <button
              ...
              style={{
                ...commStyles.tabBase,
                background: active ? "linear-gradient(135deg,#3b791e,#3b791e)" : "#e8f5e9",
      ...
      <div style={commStyles.listArea}>
        ...
        {fetching ? (
          ...
          <div style={commStyles.emptyState}>
            <div style={commStyles.emptyIcon}><EmptyIcon size={38} strokeWidth={1.7} /></div>
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: 14, paddingRight: 40 }}>
                ...
                <div>
                  <div style={{ display: "flex", gap: 6, marginBottom: 6, flexWrap: "wrap" }}>
                    {viewingItem.pinned && <span style={{ background: "rgba(255,255,255,0.25)", padding: "2px 8px", borderRadius: 8, fontSize: 9, fontWeight: 900, color: "#fff", letterSpacing: "0.08em", display:'inline-flex', alignItems:'center', gap:4 }}><Pin size={9} /> PINNED</span>}
              ...
              {isAdminUser(commUser) && (
                <div style={{ display: "flex", gap: 10, marginTop: 28, flexWrap: "wrap" }}>
                  ...
                  <button
                    ...
                    style={{ display: "flex", alignItems: "center", gap: 7, padding: "9px 18px", borderRadius: 11, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", border: "none", background: "linear-gradient(135deg,#3b791e,#3b791e)", color: "#fff" }}>
                    <Pencil size={13} /> Edit
                  </button>
                  <button
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 20, width: "100%", maxWidth: 500, boxShadow: "0 24px 64px rgba(0,0,0,0.18)", border: "1px solid rgba(59,121,30,0.15)", overflow: "hidden" }}>
            ...
            <form onSubmit={handleSave} style={{ padding: "22px 24px" }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                ...
                <button type="submit" style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "10px 0", borderRadius: 10, border: "none", background: "linear-gradient(135deg,#3b791e,#3b791e)", color: "#fff", fontSize: 13, fontWeight: 800, cursor: "pointer", fontFamily: "inherit", boxShadow: "0 2px 10px rgba(59,121,30,0.35)" }}>
  ...
  return (
    <div>
      <div style={{ background: 'var(--grad-dark)', borderRadius: 20, padding: '28px 28px 20px', marginBottom: 22, display: 'flex', alignItems: 'center', gap: 20 }}>
        <div style={{ width: 64, height: 64, borderRadius: 18, background: 'var(--grad-gold)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: 26, color: '#fff', fontFamily: 'Plus Jakarta Sans,sans-serif', boxShadow: '0 6px 20px rgba(0,0,0,0.2)', flexShrink: 0 }}>
          {initials}
        </div>
        <div>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
            <span className="v-badge v-badge-green" style={{ background: 'rgba(80,152,32,0.2)', color: '#a7f3d0' }}>Franchisee</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        <div className="v-card" style={{ padding: '22px 24px' }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="v-form-group">
              <label className="v-form-label">Personal Email <span style={{ textTransform: 'none', fontWeight: 500, color: '#9CA89C' }}>(for OTP)</span></label>
        ...
        <div className="v-card" style={{ padding: '22px 24px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'rgba(59,121,30,0.06)', border: '1.5px solid rgba(59,121,30,0.15)', borderRadius: 12, marginBottom: 20 }}>
      ...
      {showOtpModal && (
        <div className="v-modal-overlay">
          <div className="v-modal" style={{ maxWidth: 440 }} onClick={e => e.stopPropagation()}>
            <div style={{ width: 60, height: 60, borderRadius: 16, background: 'var(--grad-main)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', color:'#fff', boxShadow: '0 6px 20px rgba(59,121,30,.3)' }}><Lock size={27} /></div>
            <h2 className="v-modal-title" style={{ textAlign: 'center' }}>Verify OTP</h2>
            <p style={{ color: '#9CA89C', fontSize: 13, textAlign: 'center', margin: '8px 0 20px', fontFamily: 'Plus Jakarta Sans,sans-serif' }}>
...
function UIModal({ modal, onClose, onConfirm }) {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(13,43,30,0.45)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:3000, padding:20, backdropFilter:"blur(4px)" }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:C.white, borderRadius:16, width:"100%", maxWidth:440, boxShadow:"0 24px 64px rgba(0,0,0,0.16)", border:`1px solid ${hc.border}`, fontFamily:"Plus Jakarta Sans,sans-serif", overflow:"hidden" }}>
...
function DeleteConfirmModal({ item, deleting, onConfirm, onCancel }) {
  ...
  return (
    <div onClick={onCancel} style={{ position:"fixed", inset:0, background:"rgba(13,43,30,0.45)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:2500, padding:20, backdropFilter:"blur(4px)" }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:C.white, borderRadius:16, width:"100%", maxWidth:420, boxShadow:"0 24px 64px rgba(0,0,0,0.16)", border:`1px solid #fecaca`, fontFamily:"Plus Jakarta Sans,sans-serif", overflow:"hidden" }}>
...
function BatchTransferHistoryModal({ batch, ingredient, apiUrl, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(13,43,30,0.5)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:2800, padding:20, backdropFilter:"blur(4px)" }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:C.white, borderRadius:18, width:"100%", maxWidth:560, maxHeight:"80vh", display:"flex", flexDirection:"column", boxShadow:"0 24px 64px rgba(0,0,0,0.18)", border:`1px solid ${C.border}`, fontFamily:"Plus Jakarta Sans,sans-serif", overflow:"hidden" }}>
...
function ImportLoadingModal({ visible, progress }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(13,43,30,0.55)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:3500, padding:20, backdropFilter:"blur(6px)" }}>
      ...
      <div style={{ background:C.white, borderRadius:18, padding:"32px 36px", width:"100%", maxWidth:380, boxShadow:"0 28px 70px rgba(0,0,0,0.22)", border:`1px solid ${C.greenMid}`, fontFamily:"Plus Jakarta Sans,sans-serif", textAlign:"center" }}>
...
function ActivityLogPanel({ log, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(13,43,30,0.5)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:2000, padding:20, backdropFilter:"blur(4px)" }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:C.white, borderRadius:18, padding:"28px 32px", width:"100%", maxWidth:780, maxHeight:"82vh", display:"flex", flexDirection:"column", boxShadow:"0 24px 64px rgba(0,0,0,0.18)", border:"1px solid rgba(0,168,76,0.15)", fontFamily:"Plus Jakarta Sans,sans-serif" }}>
...
function ReceiveStockModal({ brandDef, brandItems, initialProduct, apiUrl, userName, userRole, onClose, onDone, showUiModal, setToast }) {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(13,43,30,0.5)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:2200, padding:20, backdropFilter:"blur(4px)" }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:C.white, borderRadius:20, width:"100%", maxWidth:560, maxHeight:"90vh", overflowY:"auto", boxShadow:"0 24px 64px rgba(0,0,0,0.20)", fontFamily:"Plus Jakarta Sans,sans-serif" }}>
...
function BatchDeleteHistoryPanel({ history, restoringId, onRestore, onClose }) {
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(13,43,30,0.55)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:3000, padding:20, backdropFilter:"blur(5px)" }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:C.white, borderRadius:20, padding:"26px 30px", width:"100%", maxWidth:660, maxHeight:"80vh", display:"flex", flexDirection:"column", boxShadow:"0 24px 64px rgba(0,0,0,0.20)", border:"1px solid #fecaca", fontFamily:"Plus Jakarta Sans,sans-serif" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
...
function BatchEditModal({ ingredient, batch, onClose, onSave, saving }) {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(13,43,30,0.55)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:2600, padding:20, backdropFilter:"blur(5px)" }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:C.white, borderRadius:18, width:"100%", maxWidth:560, maxHeight:"90vh", overflowY:"auto", boxShadow:"0 24px 64px rgba(0,0,0,0.20)", fontFamily:"Plus Jakarta Sans,sans-serif" }}>
...
const restoreBatch = async (entry) => {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:2000, padding:20, backdropFilter:"blur(4px)" }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:"#fff", borderRadius:20, width:"100%", maxWidth:700, maxHeight:"88vh", display:"flex", flexDirection:"column", boxShadow:"0 24px 64px rgba(0,0,0,0.18)", fontFamily:"Plus Jakarta Sans,sans-serif", overflow:"hidden" }}>
        ...
        <div style={{ padding:"18px 24px", background:"linear-gradient(135deg,#00c853,#00897b)", color:"#fff", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          ...
          <button onClick={onClose} aria-label="Close batch details" style={{ background:"rgba(255,255,255,0.2)", border:"none", color:"#fff", borderRadius:"50%", width:32, height:32, display:"flex", alignItems:"center", justifyContent:"center" }}><XIcon size={16}/></button>
        </div>
...
/* ─────────────────────────────────────────────────────────────────────────
   MAIN COMPONENT
───────────────────────────────────────────────────────────────────────── */
function StockInventoryContent({ user, brands: propBrands = [], initialFocus = null }) {
  ...
  const ownBrandObj = useMemo(() => {
    ...
    const accountBrand = String(user?.brand || user?.brand_name || user?.brandName || "").trim();
    if (accountBrand) {
      const exact = brandList.find(b => String(b.name || "").trim().toLowerCase() === accountBrand.toLowerCase());
      if (exact) return exact;
    }
    const owners = brandList.filter(b =>
      (b.branches || []).some(br => String(typeof br === "string" ? br : br?.name || "").trim().toLowerCase() === userBranch.trim().toLowerCase())
    );
...
const emptyForm = useCallback(() => ({
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const rows = isAdmin ? allRows : allRows.filter(item => {
        const sameBranch = String(item.branch || "").trim().toLowerCase() === userBranch.trim().toLowerCase();
        const itemBrand = String(item.brand || "").trim();
        const sameBrand = !ownBrandObj?.name || !itemBrand || itemBrand.toLowerCase() === ownBrandObj.name.toLowerCase();
  ...
  const filtered = useMemo(() => {
    ...
    return [...items]
      .filter(i => {
        if (!isAdmin) {
          if (String(i.branch || '').trim().toLowerCase() !== userBranch.trim().toLowerCase()) return false;
          const itemBrand = String(i.brand || '').trim();
          if (ownBrandObj?.name && itemBrand && itemBrand.toLowerCase() !== ownBrandObj.name.toLowerCase()) return false;
        }
  ...
  }, [items, search, brand, branch, unitFilter, statusFilt, sort, brandList, isAdmin, userBranch, ownBrandObj]);
...
const fetchCatalogStockItems = useCallback(async () => {
  ...
  }, [isAdmin, userBranch, ownBrandName, fetchStockItems, fetchCatalogStockItems]);
  useEffect(() => {
...
const inventoryForCatalogue = useMemo(() => {
    ...
    */
    ...
    return combined.filter(item => {
      const sameBranch = String(item.branch || "").trim().toLowerCase() === userBranch.trim().toLowerCase();
      const itemBrand = String(item.brand || branchToBrand[item.branch] || "").trim();
      const sameBrand = !ownBrandName || itemBrand.toLowerCase() === ownBrandName.toLowerCase();
      return sameBranch && sameBrand;
    });
  }, [inventory, branchToBrand, brandList, catalogStockItems, isAdmin, userBranch, ownBrandName]);
