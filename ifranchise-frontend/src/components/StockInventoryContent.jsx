...
function defaultBulkQtyForUnit(unit) {
  if (["g", "ml"].includes(unit)) return 1000;
  if (unit === "L") return 200;
  if (unit === "kg") return 50;
  if (["pc", "bottle"].includes(unit)) return 50;
  return 1;
}
...
function ReceiveStockModal({
  ...
  const bulkQty =
    Number(product?.bulk_qty) > 0
      ? Number(product.bulk_qty)
      : defaultBulkQtyForUnit(product?.unit);
  const shopPricePreview = unitCost * bulkQty * 1.12;
                  ...
                  >
                    ...
                    <div style={{ fontSize: 10, color: C.muted, marginTop: 2 }}>
                      {directProduct
                        ...
                        : `cost/unit × ${bulkQty} ${product?.unit || "unit"} + 12%`}
                    </div>
                    <div
                      style={{
                        fontSize: 18,
...
export default function StockInventoryContent({
      ...
      try {
        ...
        if (d.success) {
          ...
          if (form.listInShop && form.cost_per_unit) {
            const computedShopPrice = previewShopPrice;
            try {
  ...
  const previewBulkQty =
    parseFloat(form.bulkQty) || defaultBulkQtyForUnit(form.unit);
  const previewShopPrice = isDirectProductBrand(form.brand || currentBrandName)
    ? computeDirectSellingPrice(form.cost_per_unit)
    : Math.round(
        parseFloat(form.cost_per_unit || 0) * previewBulkQty * 1.12 * 100,
      ) / 100;
